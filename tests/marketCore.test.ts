...
describe('Production Advertisement & 20-Category Coverage Verification', () => {
  it('ensures all 20 main categories have at least 1 active advertisement', async () => {
    const { CATEGORIES } = await import('../src/data/categoriesData');
    const { INITIAL_PRODUCTS } = await import('../src/data/mockProducts');
  ...
  it('ensures all initial products follow valid category and subcategory hierarchy without dummy text', async () => {
    const { CATEGORIES } = await import('../src/data/categoriesData');
    const { INITIAL_PRODUCTS } = await import('../src/data/mockProducts');
