    ...
    <>
      ...
      <div className="hidden lg:block">
        <div className="max-w-7xl mx-auto px-2 sm:px-3 py-1 sm:py-1.5 flex flex-col gap-1 sm:gap-1.5 relative z-20 w-full">
            ...
            <div className="flex items-center gap-1 sm:gap-1.5 shrink-0 flex-wrap justify-end">
              ...
              {isLoggedIn ? (
                <div className="flex items-center gap-1 shrink-0">
                  ...
                  <button
                    onClick={() => logout()}
                    className="liquid-btn h-8 sm:h-8.5 flex items-center justify-center px-3 cursor-pointer text-xs font-black text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-950/40 border border-red-200 dark:border-red-900/50 transition shrink-0"
                    title={language === 'bn' ? 'লগ আউট' : 'Log Out'}
                  >
                    <span className="whitespace-nowrap">{language === 'bn' ? 'লগ আউট' : 'Logout'}</span>
                  </button>
                </div>
              ) : (
                <button
                  ...
                  title={language === 'bn' ? 'লগইন করুন' : 'Login'}
                >
                  <User className="w-4 h-4 text-emerald-600 shrink-0 drop-shadow-sm" />
              ...
              >
                ...
                <span className="whitespace-nowrap font-black text-white">{language === 'bn' ? 'বিজ্ঞাপন দিন' : 'Post Ad'}</span>
              </button>
          ...
          {/* DESKTOP ROW 2: Compact Search Bar + Navigation Buttons on the Same Line */}
          <div className="w-full flex items-center justify-between gap-1.5 flex-wrap">
            {/* Navigation Buttons from the Bottom Row */}
            <div className="flex items-center gap-1 sm:gap-1.5 flex-wrap shrink-0">
              <button
                onClick={() => setActiveTab('home')}
                className={`liquid-btn flex items-center justify-center gap-1.5 px-3 h-8 cursor-pointer text-xs font-black shrink-0 whitespace-nowrap ${
                  activeTab === 'home' ? 'liquid-btn-active' : ''
                }`}
              >
                <Home className="w-3.5 h-3.5 text-emerald-600 shrink-0 drop-shadow-sm" />
              ...
              <button
                ...
                className={`liquid-btn flex items-center justify-center gap-1.5 px-3 h-8 cursor-pointer text-xs font-black shrink-0 whitespace-nowrap ${
                  activeTab === 'search' && !filters.category ? 'liquid-btn-active' : ''
                }`}
              >
                <Search className="w-3.5 h-3.5 text-amber-600 shrink-0 drop-shadow-sm" />
              ...
              <button
                ...
                title="Location"
              >
                <MapPin className="w-3.5 h-3.5 text-emerald-600 shrink-0 drop-shadow-sm" />
              ...
              <button
                ...
                title="Ask AI"
              >
                <Sparkles className="w-3.5 h-3.5 text-emerald-600 shrink-0 animate-pulse drop-shadow-sm" />
                <span>{language === 'bn' ? 'আস্ক AI' : 'Ask AI'}</span>
              </button>

              <button
                onClick={() => setActiveTab('chat')}
                className={`liquid-btn flex items-center justify-center gap-1.5 px-3 h-8 cursor-pointer text-xs font-black shrink-0 whitespace-nowrap relative ${
                  activeTab === 'chat' ? 'liquid-btn-active-sky' : ''
                }`}
                title={language === 'bn' ? 'লাইভ চ্যাট ও ইনবক্স' : 'Live Chat & Inbox'}
              >
                <MessageSquare className="w-3.5 h-3.5 text-sky-600 shrink-0 drop-shadow-sm" />
              ...
              <button
                ...
                className={`liquid-btn flex items-center justify-center gap-1.5 px-3 h-8 cursor-pointer text-xs font-black shrink-0 whitespace-nowrap relative ${
                  activeTab === 'dashboard' ? 'liquid-btn-active-pink' : ''
                }`}
                title={language === 'bn' ? 'পছন্দের তালিকা' : 'Saved Wishlist'}
              >
                <Heart className="w-3.5 h-3.5 text-red-500 shrink-0 drop-shadow-sm" />
              ...
              {isAdmin && (
                <div className="flex items-center gap-1 shrink-0">
                  ...
                  >
                    <LayoutDashboard className="w-3.5 h-3.5 text-pink-600 shrink-0 drop-shadow-sm" />
            ...
            {/* Compact Search Bar with Search Button */}
            <div className="shrink-0 flex-1 min-w-[220px] max-w-xs md:max-w-sm ml-auto">
              <form
                onSubmit={handleSearchSubmit}
                className="w-full relative flex items-center"
              >
                <div className="relative w-full h-8 bg-slate-50 dark:bg-slate-800/90 border border-slate-300 dark:border-slate-700 hover:border-slate-400 focus-within:border-emerald-500 focus-within:ring-2 focus-within:ring-emerald-500/20 rounded-full text-slate-800 dark:text-white transition px-2.5 flex items-center shadow-[inset_0_1.5px_3px_rgba(0,0,0,0.06),0_2px_6px_rgba(0,0,0,0.04)] overflow-hidden">
                  ...
                  <input
                    ...
                    placeholder={
                      language === 'bn'
                        ? 'কী খুঁজছেন? যেমন: iPhone...'
                        : 'Search e.g. iPhone...'
                    }
                    className="w-full h-full text-[11px] sm:text-xs focus:outline-none text-slate-900 dark:text-white placeholder-slate-400 font-medium bg-transparent min-w-0"
                  />
                  ...
                  <div className="flex items-center gap-1 shrink-0 ml-1">
                    <button
                      ...
                      className="p-1 text-slate-500 hover:text-emerald-600 hover:bg-slate-200/60 dark:hover:bg-slate-700 rounded-full transition cursor-pointer"
                    ...
                    <button
                      type="submit"
                      className="liquid-btn liquid-btn-emerald active:scale-95 text-white font-bold px-2.5 py-0.5 transition text-[11px] flex items-center gap-1 shrink-0 cursor-pointer h-6 whitespace-nowrap"
                    >
                ...
                {isLoggedIn ? (
                  <div className="flex items-center gap-1.5 shrink-0">
                    ...
                    <button
                      ...
                      className="text-[11px] bg-red-600/30 hover:bg-red-600/40 text-red-300 font-bold px-2.5 py-1.5 rounded-xl border border-red-500/40 transition cursor-pointer active:scale-95 flex items-center justify-center"
                      title={language === 'bn' ? 'লগ আউট' : 'Log Out'}
                    >
            ...
            <div className="pt-4 mt-4 border-t border-slate-800 text-center space-y-2">
              {isLoggedIn && (
                <button
                  ...
                  className="w-full py-2.5 bg-red-600/20 hover:bg-red-600/30 text-red-400 font-bold text-xs rounded-xl border border-red-500/30 flex items-center justify-center cursor-pointer transition active:scale-95"
                  title={language === 'bn' ? 'লগ আউট' : 'Log Out'}
                >
