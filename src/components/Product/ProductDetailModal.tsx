...
import { isPhoneVisibleToBuyers, formatPhoneDisplay, maskPhoneNumber, validateBangladeshiPhone } from '../../utils/phoneUtils';
import {
  Phone,
  MessageSquare,
  ShieldCheck,
...
export const ProductDetailModal: React.FC = () => {
  ...
  const handleSendOffer = (e: React.FormEvent) => {
    ...
    if (!threadId) return; // not logged in — auth modal was opened instead
    sendMessage(
      threadId,
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white dark:bg-slate-900 rounded-xl p-6 border border-slate-200 dark:border-slate-800 shadow-2xs space-y-4">
              ...
              <div className="space-y-2.5 pt-2">
                ...
                {!isPhoneVisibleToBuyers(selectedProduct) ? (
                  <div className="space-y-2">
                    <div className="w-full bg-amber-50/90 dark:bg-amber-950/60 text-amber-900 dark:text-amber-200 font-bold py-3 px-4 rounded-xl border border-amber-200 dark:border-amber-800 flex items-center justify-center gap-2 text-xs shadow-2xs">
                    ...
                    {/* If current viewer is owner or admin, show their private number indicator + edit shortcut */}
                    {(isOwner || isAdmin) && selectedProduct.seller?.phone && (
                      <div className="p-2.5 bg-slate-50 dark:bg-slate-800/80 border border-dashed border-slate-300 dark:border-slate-700 rounded-xl flex items-center justify-between text-xs">
                        <div className="flex items-center gap-1.5 text-slate-600 dark:text-slate-300 font-mono">
                          ...
                          <span className="font-bold">{language === 'bn' ? 'আপনার পোস্টের নম্বর:' : 'Your Phone:'}</span>
                          <span>{formatPhoneDisplay(selectedProduct.seller.phone)}</span>
                        </div>
                        {isOwner && (
                          <button
                            onClick={() => {
                            ...
                            className="text-[11px] font-extrabold text-pink-600 dark:text-pink-400 hover:underline flex items-center gap-1 cursor-pointer"
                          >
                            <Edit3 className="w-3 h-3" />
                            <span>{language === 'bn' ? 'নম্বর অন করুন' : 'Turn ON'}</span>
                          </button>
                        )}
                      </div>
                    )}
                  ...
                  <div className="space-y-2">
                    {!showPhone ? (
                      ...
                      >
                        ...
                        <span>
                          {language === 'bn'
                            ? `ফোন নম্বর দেখুন (${maskPhoneNumber(selectedProduct.seller?.phone)})`
                            : `Show Phone Number (${maskPhoneNumber(selectedProduct.seller?.phone)})`}
                        </span>
                      </button>
                    ) : (
                      <div className="space-y-1.5 animate-in fade-in">
                        <a
                          href={`tel:${(selectedProduct.seller?.phone || '').replace(/[^\d+]/g, '')}`}
                          className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-black py-3 px-4 rounded-xl transition flex items-center justify-center gap-2 text-sm shadow-md cursor-pointer"
                        >
                          <Phone className="w-4 h-4 fill-white" />
                          <span>
                            {language === 'bn' ? 'সরাসরি কল করুন: ' : 'Call Seller: '}
                            {formatPhoneDisplay(selectedProduct.seller?.phone || '')}
                          </span>
                        </a>
                        <div className="flex items-center justify-between px-2 text-[11px] text-slate-500 dark:text-slate-400 font-medium">
                          <span>{language === 'bn' ? 'কল করতে সরাসরি ট্যাপ করুন' : 'Tap to call seller directly'}</span>
                          <button
                            onClick={() => {
                              if (selectedProduct.seller?.phone) {
                                navigator.clipboard.writeText(selectedProduct.seller.phone);
                                alert(language === 'bn' ? 'নম্বর কপি করা হয়েছে' : 'Phone number copied!');
                              }
                            }}
                            className="text-pink-600 dark:text-pink-400 hover:underline flex items-center gap-1 font-bold cursor-pointer"
                          >
