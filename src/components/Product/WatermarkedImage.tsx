...
export const WatermarkedImage: React.FC<WatermarkedImageProps> = ({
  ...
  const fallbackSrc = 'https://images.unsplash.com/photo-1526170375885-4d8ecf77b99f?auto=format&fit=crop&w=380&q=65';
  const effectiveSrc = hasError || !src ? fallbackSrc : src;
  const optimizedSrc = getOptimizedImageUrl(effectiveSrc, 380);
  ...
  return (
    ...
    >
      {/* Instant visual shimmer skeleton while photo is decoding */}
      {!isLoaded && !hasError && (
      ...
      <img
        ...
        onContextMenu={handleContextMenu}
        onDragStart={handleDragStart}
        className={`pointer-events-none select-none transition-opacity duration-300 ${isLoaded ? 'opacity-100' : 'opacity-0'} ${imgClassName}`}
        style={{
          WebkitUserSelect: 'none',
          userSelect: 'none',
          WebkitTouchCallout: 'none',
