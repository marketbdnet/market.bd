...
export const ClockSettingsAdminPanel: React.FC = () => {
  ...
  const [textColor, setTextColor] = useState<string>(
    clockSettings.textColor || '#00FF66'
  );
  const [dateTextColor, setDateTextColor] = useState<string>(
    clockSettings.dateTextColor || '#FFFFFF'
  );
  const [bgColor, setBgColor] = useState<string>(
    clockSettings.bgColor || '#090d16'
  );
  const [borderColor, setBorderColor] = useState<string>(
    clockSettings.borderColor || '#ef4444'
  );
  ...
  >(clockSettings.fontWeight || 'black');
  ...
  const [glowEffect, setGlowEffect] = useState<boolean>(
    clockSettings.glowEffect !== false
  );

  const [statusMsg, setStatusMsg] = useState('');

  // Preset Color Palettes - High Contrast, Sparkling & Neon Vivid
  const colorPresets = [
    { name: '✨ Ultra Neon Lime (চকচকা নিয়ন সবুজ)', hex: '#00FF66' },
    { name: '✨ Sparkling Sun Gold (চকচকা সোনালী)', hex: '#FFEA00' },
    { name: '✨ Diamond Crystal White (চকচকা স্ফটিক সাদা)', hex: '#FFFFFF' },
    { name: '✨ Electric Cyber Cyan (চকচকা সায়ান)', hex: '#00F5FF' },
    { name: '✨ Neon Hot Pink (চকচকা নিয়ন পিংক)', hex: '#FF1493' },
    { name: '✨ Vivid Sun Orange (চকচকা কমলা)', hex: '#FF6600' },
    { name: '✨ Emerald Laser (উজ্জ্বল পান্না)', hex: '#10E886' },
    { name: '✨ Bright Royal Amber (উজ্জ্বল অ্যাম্বার)', hex: '#FBBF24' },
  ];

  // Quick 1-click ultra-shiny mode applier
  const applyUltraShinyPreset = () => {
    setTextColor('#00FF66');
    setDateTextColor('#FFFFFF');
    setBgColor('#090d16');
        ...
        <div className="bg-slate-950/80 p-4 rounded-2xl border border-slate-800 shadow-inner flex flex-col items-center gap-1.5 shrink-0 min-w-[240px]">
          ...
          <div
            style={{
              ...
              boxShadow: `0 0 10px ${borderColor}55, inset 0 0 8px rgba(0,0,0,0.85)`
            }}
          ...
          >
            {showPulseIcon && (
              <Clock
                ...
                style={{
                  color: textColor,
                  filter: glowEffect ? `drop-shadow(0 0 4px ${textColor})` : undefined
                }}
              />
            )}
            <div className="flex flex-col text-left leading-none justify-center gap-1">
              <span
                style={{
                  color: dateTextColor,
                  textShadow: '0 1px 3px rgba(0,0,0,0.95), 0 0 4px rgba(255,255,255,0.45)'
                }}
                className="text-[10px] font-bold tracking-tight whitespace-nowrap leading-tight"
              >
              ...
              <span
                style={{
                  ...
                  textShadow: glowEffect
                    ? `0 0 10px ${textColor}, 0 0 4px ${textColor}, 0 1px 2px rgba(0,0,0,0.95)`
      ...
      <form onSubmit={handleSave} className="space-y-6">
        ...
        <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 p-6 shadow-xl space-y-4">
          ...
          <div className="space-y-3">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
              ...
              <button
                type="button"
                onClick={applyUltraShinyPreset}
                className="flex items-center gap-1.5 py-1 px-3 bg-gradient-to-r from-emerald-600 via-teal-500 to-green-500 text-white rounded-xl text-xs font-black shadow-md hover:shadow-lg active:scale-95 transition cursor-pointer self-start sm:self-auto"
              >
                <Sparkles className="w-3.5 h-3.5 animate-spin" />
                <span>{language === 'bn' ? '⚡ ১-ক্লিকে সুপার চকচকা নিয়ন মোড' : '⚡ 1-Click Ultra Shiny Neon'}</span>
              </button>
            </div>
            <div className="flex flex-wrap gap-2">
        ...
        <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 p-6 shadow-xl space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            {/* Toggle 4: Glowing / Shiny Luminous Effect */}
            <label className="flex items-center gap-3 p-3 bg-emerald-500/10 dark:bg-emerald-950/30 rounded-2xl border border-emerald-500/30 cursor-pointer">
              <input
                type="checkbox"
                checked={glowEffect}
              ...
              <div className="flex flex-col">
                <span className="text-xs font-black text-emerald-700 dark:text-emerald-300 flex items-center gap-1">
                  ...
                  {language === 'bn' ? 'চকচকা নিয়ন এফেক্ট' : 'Shiny Luminous Glow'}
                </span>
                <span className="text-[10px] text-slate-500 dark:text-slate-400">
                  {language === 'bn' ? 'টেক্সট চকচকা উজ্জ্বল দেখাবে' : 'Vivid bright glowing digits'}
                </span>
              </div>
            </label>
          </div>
