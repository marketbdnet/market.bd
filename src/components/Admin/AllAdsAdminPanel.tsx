...
import { isProductPublicActive, isProductPending, isProductRejected, isProductSold } from '../../utils/productStatus';
...
export const AllAdsAdminPanel: React.FC = () => {
  ...
  const filteredAds = useMemo(() => {
    return products.filter(ad => {
      // Status filter
      if (statusFilter !== 'all') {
        if (statusFilter === 'active' && !isProductPublicActive(ad)) return false;
        else if (statusFilter === 'pending' && !isProductPending(ad)) return false;
        else if (statusFilter === 'rejected' && !isProductRejected(ad)) return false;
        else if (statusFilter === 'sold' && !isProductSold(ad)) return false;
      }
  ...
  const activeCount = products.filter(p => isProductPublicActive(p)).length;
  const pendingCount = products.filter(p => isProductPending(p)).length;
  const rejectedCount = products.filter(p => isProductRejected(p)).length;
  const soldCount = products.filter(p => isProductSold(p)).length;
    ...
    <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 border border-slate-200 dark:border-slate-800 shadow-2xs space-y-6 text-slate-900 dark:text-white">
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-3 bg-slate-50 dark:bg-slate-800/60 p-3.5 rounded-2xl border border-slate-200 dark:border-slate-700">
        <div className="relative flex-1 w-full">
          ...
          <input
            type="search"
            name="admin_all_ads_search_filter"
            id="admin-all-ads-search-filter"
            autoComplete="off"
            autoCorrect="off"
            autoCapitalize="none"
            spellCheck="false"
                    ...
                    <div className="flex items-center gap-3 text-[11px] text-slate-500 dark:text-slate-400 flex-wrap">
                      ...
                      {ad.seller?.phone && (
                        <span className="flex items-center gap-1.5 font-mono text-slate-700 dark:text-slate-300">
                          ...
                          {ad.showPhoneNumber || (!ad.seller?.hidePhone && ad.seller?.showPhoneNumber !== false) ? (
