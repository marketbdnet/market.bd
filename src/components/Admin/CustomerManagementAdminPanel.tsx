...
export const CustomerManagementAdminPanel: React.FC = () => {
  ...
  const [directMsgCustomer, setDirectMsgCustomer] = useState<UserProfile | null>(null);
  const [directMsgChannel, setDirectMsgChannel] = useState<'sms' | 'email'>('sms');
  const [directMsgSubject, setDirectMsgSubject] = useState('MarketBD.Net নোটিশ');
  ...
  // Send Direct SMS or Email to Customer
  const handleSendDirectMsg = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!directMsgCustomer) return;
    if (!directMsgBody.trim()) {
      alert(language === 'bn' ? 'মেসেজ বার্তা লিখুন।' : 'Please enter message content.');
      return;
    }

    setIsSendingDirectMsg(true);
    try {
      if (directMsgChannel === 'sms') {
        if (!directMsgCustomer.phone) {
          alert(language === 'bn' ? 'গ্রাহকের ফোন নম্বর নেই।' : 'Customer has no phone number.');
          setIsSendingDirectMsg(false);
          return;
        }
        await sendCustomSms(directMsgCustomer.phone, directMsgBody.trim());
      } else {
        if (!directMsgCustomer.email) {
          alert(language === 'bn' ? 'গ্রাহকের ইমেইল ঠিকানা নেই।' : 'Customer has no email address.');
          setIsSendingDirectMsg(false);
          return;
        }
        await sendCustomerPromotionalEmail(directMsgCustomer.email, directMsgSubject.trim(), directMsgBody.trim());
      }

      setIsSendingDirectMsg(false);
      setSingleMsgDeliverySuccess({
        recipient: directMsgChannel === 'sms' ? directMsgCustomer.phone! : directMsgCustomer.email!,
        recipientName: directMsgCustomer.name,
        messageText: directMsgBody.trim(),
        channel: directMsgChannel
      });
      setDirectMsgCustomer(null);
      setDirectMsgBody('');
    } catch (err: any) {
      setIsSendingDirectMsg(false);
    ...
    <div className="space-y-6">
      ...
      {singleMsgDeliverySuccess && (
        <MessageDeliverySuccessBanner
          ...
          recipientName={singleMsgDeliverySuccess.recipientName}
      ...
      <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 border border-slate-200 dark:border-slate-800 shadow-sm">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2.5 flex-wrap">
            <button
              ...
              className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-gradient-to-r from-pink-600 via-rose-600 to-indigo-600 hover:from-pink-700 hover:to-indigo-700 active:scale-95 text-white font-extrabold text-xs shadow-md shadow-pink-600/25 transition-all cursor-pointer"
            >
              <Megaphone className="w-4 h-4 animate-bounce" />
              <span>{language === 'bn' ? '📢 বাল্ক প্রমোশন পাঠান (SMS / Email)' : '📢 Bulk Broadcast (SMS/Email)'}</span>
            </button>
            <button
        ...
        {/* SMS Quota Usage Strip */}
        <div className="mt-4 pt-4 border-t border-slate-100 dark:border-slate-800 flex flex-wrap items-center justify-between gap-3 p-3 bg-gradient-to-r from-pink-50/60 via-slate-50 to-indigo-50/60 dark:from-slate-800/80 dark:to-slate-800/60 rounded-2xl">
          <div className="flex items-center gap-2 text-xs font-black text-slate-800 dark:text-slate-200">
            <Zap className="w-4 h-4 text-pink-600" />
            <span>{language === 'bn' ? 'এসএমএস ব্যালেন্স ট্র্যাকার:' : 'SMS Balance Tracker:'}</span>
          </div>
          <div className="flex flex-wrap items-center gap-2.5 text-xs font-mono">
            <span className="bg-purple-100 dark:bg-purple-950/80 text-purple-900 dark:text-purple-200 px-2.5 py-1 rounded-xl border border-purple-300 dark:border-purple-800">
              🔤 মাসকিং ব্যালেন্স: <b>{getMaskingRemainingSms()}টি</b>
            </span>
            <span className="bg-blue-100 dark:bg-blue-950/80 text-blue-900 dark:text-blue-200 px-2.5 py-1 rounded-xl border border-blue-300 dark:border-blue-800">
              🔢 নন-মাসকিং ব্যালেন্স: <b>{getNonMaskingRemainingSms()}টি</b>
            </span>
            <span className="bg-emerald-600 text-white px-2.5 py-1 rounded-xl shadow-xs">
              💳 সর্বমোট অবশিষ্ট: <b>{getRemainingPurchasedSms()}টি</b>
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-3xl p-4 sm:p-5 border border-slate-200 dark:border-slate-800 shadow-sm space-y-4">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-3">
          <div className="relative flex-1 w-full">
            ...
            <input
              type="text"
              name="admin_customer_search_query_field"
              id="admin-customer-search-query-field"
                    ...
                    >
                      ...
                      <td className="py-4 px-4 text-right">
                        <div className="flex items-center justify-end gap-1.5">
                          {/* Send Direct SMS / Email Button */}
                          <button
                            onClick={() => {
                              setDirectMsgCustomer(user);
                              setDirectMsgChannel(user.phone ? 'sms' : 'email');
                              setDirectMsgBody(`প্রিয় ${user.name || 'গ্রাহক'}, MarketBD.Net-এ আপনাকে স্বাগতম। আপনার জন্য আকর্ষণীয় অফার অপেক্ষা করছে!`);
                            }}
                            className="p-2 rounded-xl bg-pink-50 hover:bg-pink-100 dark:bg-pink-950/60 text-pink-600 dark:text-pink-400 hover:text-pink-700 transition-colors cursor-pointer"
                            title={language === 'bn' ? 'গ্রাহককে এসএমএস / ইমেইল পাঠান' : 'Send SMS/Email to Customer'}
                          >
                ...
                >
                  ...
                  <div className="flex items-center justify-between gap-2 pt-1">
                    <button
                      ...
                      className="p-2 rounded-xl bg-pink-50 hover:bg-pink-100 dark:bg-pink-950/60 text-pink-600 dark:text-pink-400 font-bold text-xs inline-flex items-center justify-center cursor-pointer"
                      title={language === 'bn' ? 'এসএমএস / ইমেইল বার্তা পাঠান' : 'Send SMS/Email'}
                    >
                      <Send className="w-4 h-4" />
                    </button>
      ...
      {/* DIRECT MESSAGE MODAL TO SPECIFIC CUSTOMER */}
      {directMsgCustomer && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs">
          <div className="bg-white dark:bg-slate-900 rounded-2xl max-w-md w-full p-5 shadow-2xl border border-slate-200 dark:border-slate-800 animate-in zoom-in-95 space-y-4">
            <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
              <div className="flex items-center gap-2">
                <div className="p-2 bg-pink-100 dark:bg-pink-950/80 text-pink-600 rounded-xl">
                  {directMsgChannel === 'sms' ? <Smartphone className="w-5 h-5" /> : <Mail className="w-5 h-5" />}
                </div>
                <div>
                  <h3 className="font-extrabold text-sm text-slate-900 dark:text-white">
                    {language === 'bn' ? 'গ্রাহককে সরাসরি বার্তা প্রেরণ' : 'Send Direct Message to Customer'}
                  </h3>
                  <p className="text-xs text-slate-500 dark:text-slate-400">
                    {directMsgCustomer.name} ({directMsgChannel === 'sms' ? directMsgCustomer.phone : directMsgCustomer.email})
                  </p>
                </div>
              </div>
              <button
                onClick={() => setDirectMsgCustomer(null)}
            ...
            <div className="grid grid-cols-2 gap-2">
              <button
                type="button"
                onClick={() => setDirectMsgChannel('sms')}
                disabled={!directMsgCustomer.phone}
                className={`py-2 px-3 rounded-xl text-xs font-bold flex items-center justify-center gap-1.5 border cursor-pointer transition ${
                  directMsgChannel === 'sms'
                    ? 'bg-pink-600 text-white border-pink-600 shadow-xs'
                    : 'bg-slate-50 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border-slate-200 dark:border-slate-700 disabled:opacity-40'
                }`}
              >
                <Smartphone className="w-3.5 h-3.5" />
                <span>মোবাইল SMS {directMsgCustomer.phone ? '' : '(অনুপলব্ধ)'}</span>
              </button>
              <button
                type="button"
                onClick={() => setDirectMsgChannel('email')}
                disabled={!directMsgCustomer.email}
                className={`py-2 px-3 rounded-xl text-xs font-bold flex items-center justify-center gap-1.5 border cursor-pointer transition ${
                  directMsgChannel === 'email'
                    ? 'bg-indigo-600 text-white border-indigo-600 shadow-xs'
                    : 'bg-slate-50 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border-slate-200 dark:border-slate-700 disabled:opacity-40'
                }`}
              >
                <Mail className="w-3.5 h-3.5" />
                <span>ইমেইল {directMsgCustomer.email ? '' : '(অনুপলব্ধ)'}</span>
              </button>
            </div>
            ...
            <form onSubmit={handleSendDirectMsg} className="space-y-3">
              {directMsgChannel === 'email' && (
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 text-xs rounded-xl border border-indigo-200 dark:border-indigo-800 bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white font-bold"
                  />
                </div>
              )}
              ...
              <div className="flex flex-wrap gap-1">
                <button
                  type="button"
                  onClick={() => setDirectMsgBody(`প্রিয় ${directMsgCustomer.name || 'গ্রাহক'}, আপনার অ্যাকাউন্টের ভেরিফিকেশন সফল হয়েছে। শুভেচ্ছাান্তে MarketBD.Net`)}
                  className="text-[10px] px-2 py-0.5 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 rounded hover:bg-slate-200 cursor-pointer"
                >
                  ভেরিফিকেশন কনফার্মেশন
                </button>
                <button
                  type="button"
                  onClick={() => setDirectMsgBody(`প্রিয় ${directMsgCustomer.name || 'গ্রাহক'}, আজই আকর্ষণীয় ডিসকাউন্ট পেতে ভিজিট করুন https://marketbd.net`)}
                  className="text-[10px] px-2 py-0.5 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 rounded hover:bg-slate-200 cursor-pointer"
                >
                  ডিসকাউন্ট অফার
                </button>
              ...
              <div className="flex items-center justify-end gap-2 pt-2">
                ...
                <button
                  ...
                  className="px-5 py-2 bg-gradient-to-r from-pink-600 to-indigo-600 hover:from-pink-700 hover:to-indigo-700 text-white text-xs font-extrabold rounded-xl shadow-md cursor-pointer flex items-center gap-1.5 disabled:opacity-50"
                >
                  {isSendingDirectMsg ? (
      ...
      {/* BULK BROADCAST TO ALL REGISTERED CUSTOMERS MODAL */}
