...
import { sendRealSmsOtp, sendEmailPasswordReset, fetchSmsBalance, getSmsSentCount } from '../../services/smsService';
...
export const LiveProductionStatusPanel: React.FC = () => {
  ...
  const [smsSenderId, setSmsSenderId] = useState<string>(
    storage.getItem('marketbd_sms_sender_id') || 'marketbd.net'
  );
  ...
  const [smsBalance, setSmsBalance] = useState<string>(
    storage.getItem('marketbd_last_known_balance') || '—'
  );
  ...
  const [gscToken, setGscToken] = useState<string>(
    storage.getItem('marketbd_gsc_token') || 'mpNzVu_LTHIFj-3DBl1A_eyHfbuX4owV4DRpw2nDxSw'
  );
  ...
  const handleSaveSettings = () => {
    ...
    const cleanSender = smsSenderId.trim();
    setSmsApiKey(cleanKey);
    setSmsSenderId(cleanSender);
    ...
    storage.setItem('marketbd_sms_sender_id', cleanSender);
  ...
  const handleSendTestSms = async () => {
    ...
    const result = await sendRealSmsOtp(testPhone.trim(), '889922', {
  ...
  const stepsList = [
    ...
    {
      id: 8,
      title: language === 'bn' ? 'Dynamic SEO, OpenGraph & Automatic Sitemap' : 'Dynamic Marketplace SEO Engine',
          ...
          <div className="bg-slate-950 p-4 rounded-2xl border border-slate-800 space-y-4">
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className="text-[10px] px-2 py-0.5 bg-slate-900 border border-slate-700 hover:border-emerald-500 text-slate-300 rounded-lg flex items-center gap-1 cursor-pointer"
              >
                <RefreshCw className={`w-3 h-3 ${isCheckingBalance ? 'animate-spin text-emerald-400' : ''}`} />
                <span>{language === 'bn' ? 'ব্যালেন্স চেক' : 'Balance'}</span>
              </button>
            </div>

            {/* Quick SMS Balance & Sent Count */}
            <div className="grid grid-cols-2 gap-2 p-2.5 bg-slate-900/90 rounded-xl border border-slate-800">
              <div>
                <span className="text-[9px] uppercase tracking-wider text-slate-500 font-bold block">
                  {language === 'bn' ? 'মোট খরচ SMS' : 'Total Sent'}
                </span>
                <span className="text-sm font-black text-pink-400 font-mono">
                  {smsSentCount} <span className="text-[10px] font-normal text-slate-400">টি</span>
                </span>
              </div>
              <div>
                <span className="text-[9px] uppercase tracking-wider text-slate-500 font-bold block">
                  {language === 'bn' ? 'অবশিষ্ট ব্যালেন্স' : 'Balance'}
                </span>
                <span className="text-sm font-black text-emerald-400 font-mono">
                  {smsBalance.startsWith('৳') ? smsBalance : `৳ ${smsBalance}`}
                </span>
              </div>
            </div>
            ...
            <div className="space-y-3">
              <div>
                ...
                <input
                  type="text"
                  list="sms-provider-options"
                  value={smsProvider}
                  onChange={e => setSmsProvider(e.target.value)}
                  placeholder="bulksmsdhaka.net"
                  className="w-full bg-slate-900 border border-slate-700 text-white text-xs font-bold rounded-xl px-3 py-2 focus:outline-none focus:border-emerald-500 font-mono"
                />
                <datalist id="sms-provider-options">
                  <option value="bulksmsdhaka.net" />
                  <option value="bulksmsbd.net" />
                  <option value="greenweb.com.bd" />
                  <option value="steadfast.com.bd" />
                </datalist>
              </div>
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="mpNzVu_LTHIFj-3DBl1A_eyHfbuX4owV4DRpw2nDxSw"
                  className="w-full bg-slate-900 border border-slate-700 text-white text-xs rounded-xl px-3 py-2 font-mono focus:outline-none focus:border-sky-500"
                />
              </div>
