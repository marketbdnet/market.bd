          ...
          return (
            ...
            >
              ...
              <button
                ...
                className={`w-full flex flex-col items-center justify-between p-1 rounded-xl border-2 border-pink-500 dark:border-pink-500/90 transition-all duration-200 cursor-pointer group text-center bg-white dark:bg-slate-900 shadow-xs hover:shadow-md hover:border-pink-600 hover:scale-[1.02] active:scale-95 overflow-hidden ${
                  isHovered ? 'bg-pink-50/70 dark:bg-pink-950/40 border-pink-600' : ''
                }`}
              >
                {/* Enlarged Image maximizing space inside compact frame right to borders */}
                <div className="w-full h-22 sm:h-24 flex items-center justify-center overflow-hidden rounded-lg bg-slate-50/80 dark:bg-slate-800/50 p-0.5">
                  <img
                    src={getOptimizedImageUrl(catSingleImage, 260)}
                    alt={cat.nameEn}
                    loading="eager"
                    fetchPriority="high"
                    decoding="async"
                    className="w-full h-full object-contain p-0 group-hover:scale-108 transition-transform duration-300 drop-shadow-2xs"
                    onError={(e) => {
                ...
                <span className="w-full font-extrabold text-[11px] sm:text-xs text-slate-900 dark:text-slate-100 mt-1 pb-0.5 leading-tight group-hover:text-pink-600 dark:group-hover:text-pink-400 transition-colors line-clamp-1 px-0.5">
        ...
        {categories.map(cat => {
          ...
          return (
            <button
              key={cat.id}
              onClick={() => handleCategoryClick(cat.id)}
              className="w-full flex flex-col items-center justify-between p-0.5 sm:p-1 rounded-xl border-2 border-pink-500 dark:border-pink-500/90 bg-white dark:bg-slate-900 text-center group cursor-pointer shadow-xs transition hover:border-pink-600 active:scale-95 hover:bg-pink-50/50 dark:hover:bg-slate-800/80 overflow-hidden"
            >
              {/* Category Image: Enlarged to maximize space close to card border */}
              <div className="w-full h-18 xs:h-20 sm:h-22 flex items-center justify-center overflow-hidden rounded-lg bg-slate-50/80 dark:bg-slate-800/50 p-0.5">
                <img
                  src={getOptimizedImageUrl(catSingleImage, 220)}
                  alt={cat.nameEn}
                  loading="eager"
                  fetchPriority="high"
                  decoding="async"
              ...
              <span className="w-full font-extrabold text-[10.5px] xs:text-[11px] text-slate-900 dark:text-slate-100 mt-1 pb-0.5 leading-tight group-hover:text-pink-600 dark:group-hover:text-pink-400 transition-colors line-clamp-1 px-0.5">
                {language === 'bn' ? cat.nameBn : cat.nameEn}
              </span>
            </button>
          );
