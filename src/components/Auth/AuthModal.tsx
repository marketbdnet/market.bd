...
import { sendRealSmsOtp, getStoredSmsConfig, playVoiceOtp } from '../../services/smsService';
...
export const AuthModal: React.FC = () => {
  ...
  // OTP Delivery Channel: 'sms' | 'email' | 'voice'
  const [otpChannel, setOtpChannel] = useState<'sms' | 'email' | 'voice'>('sms');
  ...
  const [resendTimer, setResendTimer] = useState(240); // 4 Minutes timer (240 seconds)
  ...
  const [loginMethod, setLoginMethod] = useState<'password_otp' | 'direct_otp'>('password_otp');
  const [smsDispatchSuccess, setSmsDispatchSuccess] = useState<boolean | null>(null);
  ...
  const handleOtpBoxChange = (index: number, val: string) => {
    ...
    if (!cleanDigits) {
      // Cleared current box
      const chars = enteredOtp.padEnd(6, ' ').split('');
      chars[index] = ' ';
      setEnteredOtp(chars.join('').replace(/\s+$/, ''));
      setOtpError('');
      return;
    }

    // If multiple digits were entered or pasted into a box
    if (cleanDigits.length > 1) {
      const chars = cleanDigits.slice(0, 6);
      setEnteredOtp(chars);
      setOtpError('');
  ...
  const handleOtpBoxKeyDown = (index: number, e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Backspace') {
      const currentChar = enteredOtp[index];
  ...
  // Deep Realtime Lookup across Local Cache, Firestore /settings/registered_users, /users collection, and credentials store
  const findUserAccountAcrossCloudAndLocal = async (identifier: string): Promise<{ exists: boolean; user?: any }> => {
    ...
    const localMatch = allUsers.find((u: any) => {
      ...
      return phoneMatch || emailMatch || u.phone === rawClean || u.email === rawClean;
    });
    if (localMatch) {
      return { exists: true, user: localMatch };
    }

    // 2. Query Firestore doc /settings/registered_users
    try {
      const regDoc = await getDoc(doc(db, 'settings', 'registered_users'));
      if (regDoc.exists()) {
        const val = regDoc.data()?.value;
        if (Array.isArray(val)) {
          const found = val.find((u: any) => {
            const uPhone = normalizePhone(u.phone);
            const uEmail = normalizeEmail(u.email);
            const phoneMatch = Boolean(cleanPhone && uPhone && (cleanPhone === uPhone || cleanPhone.endsWith(uPhone) || uPhone.endsWith(cleanPhone)));
            const emailMatch = Boolean(cleanEmail && uEmail && cleanEmail === uEmail);
          ...
          if (found) return { exists: true, user: found };
        }
      }
    } catch (e) {}
    ...
    try {
      if (cleanPhone) {
        ...
        if (!s1.empty) return { exists: true, user: s1.docs[0].data() };

        if (rawClean !== cleanPhone) {
          const q2 = query(collection(db, 'users'), where('phone', '==', rawClean));
          const s2 = await getDocs(q2);
          if (!s2.empty) return { exists: true, user: s2.docs[0].data() };
        }
      }
      if (cleanEmail) {
        const q3 = query(collection(db, 'users'), where('email', '==', cleanEmail));
        const s3 = await getDocs(q3);
        if (!s3.empty) return { exists: true, user: s3.docs[0].data() };
      }
    } catch (e) {}

    // 4. Query Firestore /settings/credentials_store doc
    try {
      const credDoc = await getDoc(doc(db, 'settings', 'credentials_store'));
      if (credDoc.exists()) {
        const store = credDoc.data()?.value || {};
        if (cleanPhone && store[cleanPhone]) return { exists: true, user: store[cleanPhone] };
        if (cleanEmail && store[cleanEmail]) return { exists: true, user: store[cleanEmail] };
        if (store[rawClean]) return { exists: true, user: store[rawClean] };
      }
    } catch (e) {}
  ...
  const handleSendOtp = async (channelOverride?: 'sms' | 'email' | 'voice') => {
    ...
    setEnteredOtp('');
    setResendTimer(240); // 4 Minutes timer (240s)

    const targetPhone = phoneOrEmail.trim();

    if (selectedChannel === 'voice') {
      const smsConfig = getStoredSmsConfig();
      playVoiceOtp(newOtp, smsConfig.voiceTemplate).catch(() => {});
      setConfirmationResult(null);
      setSmsDispatchSuccess(true);
    } else if (selectedChannel === 'sms' && isValidBDPhone(targetPhone)) {
      // 1. Dispatch Real SMS OTP to recipient via active gateway
      const smsConfig = getStoredSmsConfig();
      const smsRes = await sendRealSmsOtp(targetPhone, newOtp, smsConfig);
      setSmsDispatchSuccess(smsRes.success);
      if (!smsRes.success) {
        setOtpError(smsRes.message);
      }
      ...
      try {
        ...
        console.warn('Firebase SMS fallback notice:', err?.message || err);
        setConfirmationResult(null);
      }
    } else {
      setConfirmationResult(null);
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    if (mode === 'register') {
      ...
      // Check for single registration uniqueness across both Cloud Firestore and local storage
      setIsSendingOtp(true);
      const cloudCheck = await findUserAccountAcrossCloudAndLocal(cleanInput);
      setIsSendingOtp(false);

      if (cloudCheck.exists) {
        setErrorMsg(
          language === 'bn'
            ? '⚠️ এই মোবাইল নম্বর দিয়ে ইতোমধ্যে একটি অ্যাকাউন্ট রয়েছে! একটি নম্বর দিয়ে একবারই রেজিস্ট্রেশন করা যাবে। অনুগ্রহ করে আপনার পাসওয়ার্ড দিয়ে "লগইন করুন" অপশন ব্যবহার করুন।'
            : '⚠️ An account with this phone number already exists! Single registration allowed. Please enter your password to Log In.'
        );
        setMode('login');
        return;
      }
    ...
    // LOGIN FORM VALIDATION & OTP ENFORCEMENT
    if (mode === 'login') {
      if (!isValidBDPhone(cleanInput) && !isValidEmail(cleanInput)) {
        setErrorMsg(
          language === 'bn'
      ...
      // 1. Direct OTP Login Method (Passwordless)
      if (loginMethod === 'direct_otp') {
        setIsSendingOtp(true);
        const cloudUserResult = await findUserAccountAcrossCloudAndLocal(cleanInput);
        setIsSendingOtp(false);

        if (cloudUserResult.exists && cloudUserResult.user) {
          const matchedUser = cloudUserResult.user;
          ...
          setPendingLoginUser({
            ...
            name: matchedUser.name || 'ভেরিফাইড ইউজার',
            phone: matchedUser.phone || cleanInput,
          ...
          handleSendOtp();
          return;
        } else if (cleanInput === '01634025151' || cleanInput.toLowerCase() === 'official.marketsbd@gmail.com') {
          setPendingLoginUser({
            id: 'admin-master',
          ...
          setErrorMsg(
            language === 'bn'
              ? '❌ এই নম্বর বা ইমেইলে কোনো রেজিস্টার্ড অ্যাকাউন্ট পাওয়া যায়নি! অনুগ্রহ করে প্রথমে সাইন আপ (Sign Up) করুন।'
              : '❌ No registered account found with this phone or email! Please Sign Up first.'
          );
          return;
        }
      }

      // 2. Password + OTP (Two-Factor Authentication / 2FA)
      ...
      // Attempt 1: Direct Firebase Auth
      try {
        ...
        setIsSendingOtp(false);
        setPendingLoginUser(userProfileData);
        handleSendOtp();
        return;
      } catch (fbErr: any) {
        // Firebase Auth failed; fallback to comprehensive Cloud Firestore / Local store check
      }

      // Attempt 2: Comprehensive Cloud Firestore & Local User Lookup
      const cloudUserResult = await findUserAccountAcrossCloudAndLocal(cleanInput);
      setIsSendingOtp(false);
      ...
      if (cloudUserResult.exists && cloudUserResult.user) {
        ...
        const storedPw = matchedUser.password;
        const isPasswordCorrect = storedPw && (storedPw === password || storedPw.trim() === password.trim());

        if (isPasswordCorrect) {
          // Auto-bridge into Firebase Auth in background if needed
          try {
          ...
          // User definitely exists, but entered wrong password!
          setErrorMsg(
            language === 'bn'
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    if (resendTimer <= 0) {
      setOtpError(
        language === 'bn'
          ? '❌ ওটিপি কোডের মেয়াদ (৪ মিনিট) উত্তীর্ণ হয়েছে! অনুগ্রহ করে নতুন ওটিপি পাঠান।'
          : '❌ OTP code has expired (4-minute validity limit). Please request a new code.'
      );
      return;
    }
    ...
    if (mode === 'register') {
      // 🛡️ Extra check: Ensure account wasn't already created in another tab or device
      const doubleCheck = await findUserAccountAcrossCloudAndLocal(cleanInput);
      if (doubleCheck.exists && doubleCheck.user) {
        alert(
          language === 'bn'
            ? '⚠️ এই নম্বর দিয়ে ইতোমধ্যে অ্যাকাউন্ট তৈরি করা হয়েছে। স্বয়ংক্রিয়ভাবে আপনার অ্যাকাউন্টে লগইন করা হচ্ছে...'
            : '⚠️ An account with this number already exists. Automatically logging you in...'
        );
        login(doubleCheck.user);
        closeAuthModal();
        return;
      }
    ...
    // LOGIN MODE -> LOG USER IN AFTER SUCCESSFUL OTP VERIFICATION
    if (mode === 'login') {
      const userToLogin = pendingLoginUser;
      if (userToLogin) {
        if (userToLogin.isBlocked || userToLogin.status === 'blocked') {
          setOtpError(
            language === 'bn'
              ? `🚫 এই অ্যাকাউন্টটি এডমিন কর্তৃক ব্লক করা হয়েছে! কারণ: ${userToLogin.blockedReason || 'অ্যাডমিন পলিসি লঙ্ঘন'}`
              : `🚫 This account has been blocked by Admin! Reason: ${userToLogin.blockedReason || 'Policy violation'}`
          );
          return;
        }
        storage.setItem('marketbd_is_logged_in', 'true');
        storage.setItem('marketbd_user_role', userToLogin.role || 'seller');
        storage.setItem('marketbd_auth_user', JSON.stringify(userToLogin));
        storage.setItem('marketbd_session_created_at', String(Date.now()));
        login(userToLogin);
        setAuthStep('form');
        setEnteredOtp('');
        setPendingLoginUser(null);
      ...
      const stableUserId = auth.currentUser?.uid || matchedUser?.id || (isAdmin ? 'admin-master' : (isPhone ? `usr-phone-${cleanPhone}` : `usr-email-${cleanEmail.replace(/[^a-z0-9]/g, '_')}`));
      ...
      const userData = {
        ...
        role: isAdmin ? ('admin' as const) : ((matchedUser?.role as any) || 'seller'),
        password: matchedUser?.password || password,
        isVerified: true
      };

      // Persist to storage synchronously before invoking login to eliminate race conditions
      storage.setItem('marketbd_is_logged_in', 'true');
      storage.setItem('marketbd_user_role', userData.role);
      storage.setItem('marketbd_auth_user', JSON.stringify(userData));
      storage.setItem('marketbd_session_created_at', String(Date.now()));
          ...
          {authStep === 'form' && (
            <div className="relative z-10 animate-in fade-in duration-300">
              ...
              <div className="text-center mb-5">
                ...
                {mode === 'login' && (
                  <p className="text-xs sm:text-sm font-semibold tracking-wide text-pink-400 drop-shadow-[0_0_12px_rgba(244,114,182,0.65)] mb-1.5 animate-in fade-in duration-200">
                    Welcome to MarketBD.Net
                  </p>
                )}
              ...
              <form onSubmit={handleFormSubmit} className="space-y-3.5">
                ...
                {/* Login Method Selector: Password+OTP (2FA) vs Direct OTP */}
                {mode === 'login' && (
                  <div className="grid grid-cols-2 gap-2 p-1 bg-white/[0.04] border border-white/10 rounded-full">
                    <button
                      type="button"
                      onClick={() => {
                        setLoginMethod('password_otp');
                        setErrorMsg('');
                      }}
                      className={`py-2 px-2.5 rounded-full text-xs font-bold flex items-center justify-center gap-1.5 transition cursor-pointer ${
                    ...
                    >
                      ...
                      <span>{language === 'bn' ? 'পাসওয়ার্ড + ওটিপি' : 'Password + OTP'}</span>
                    </button>
                    <button
                      type="button"
                      onClick={() => {
                    ...
                    >
                      ...
                      <span>{language === 'bn' ? 'সরাসরি ওটিপি' : 'Direct OTP'}</span>
                    </button>
                  </div>
                )}
                ...
                {/* Password input */}
                {mode !== 'forgot' && (mode !== 'login' || loginMethod === 'password_otp') && (
                  <div className="relative">
                ...
                {mode === 'login' && loginMethod === 'password_otp' && (
                ...
                >
                  ...
                  <span className="text-sm font-bold tracking-wide">
                    {isSendingOtp ? (
                      ...
                      language === 'bn' ? 'লগইন করুন (ওটিপি যাচাই)' : 'Login with OTP'
                    )}
                  </span>
            ...
            <div className="relative z-10 space-y-4 animate-in fade-in duration-300">
              ...
              <div className="flex bg-white/[0.05] border border-white/10 p-1 rounded-full gap-1 text-xs font-bold">
                <button
                  ...
                  className={`flex-1 py-1.5 px-2.5 rounded-full transition flex items-center justify-center gap-1 cursor-pointer ${
                    otpChannel === 'sms'
                ...
                >
                  ...
                  <span>{language === 'bn' ? 'ভয়েস কল' : 'Voice'}</span>
                </button>
              ...
              <div className="text-center space-y-1.5">
                <div className="w-12 h-12 rounded-full bg-pink-500/15 text-pink-400 flex items-center justify-center mx-auto border border-pink-500/40 shadow-sm">
                  {mode === 'login' ? (
                    ...
                    <Volume2 className="w-6 h-6 animate-pulse text-purple-400" />
                  ) : (
                ...
                <h3 className="text-lg font-bold text-white">
                  {mode === 'login'
                    ? (language === 'bn' ? '🛡️ নিরাপদ ওটিপি ভেরিফিকেশন (2FA)' : '🛡️ Secure OTP Verification (2FA)')
                    : otpChannel === 'sms'
                    ? (language === 'bn' ? 'মোবাইল ওটিপি ভেরিফিকেশন' : 'Mobile OTP Verification')
                    : otpChannel === 'voice'
                    ? (language === 'bn' ? 'ভয়েস কল ওটিপি ভেরিফিকেশন' : 'Voice Call OTP Verification')
                ...
                <p className="text-xs text-zinc-400">
                  {otpChannel === 'sms' ? (
                    language === 'bn' ? (
                      <>
                        অ্যাকাউন্টের সুরক্ষায় আপনার <span className="font-bold text-pink-400 font-mono">{phoneOrEmail || '017XXXXXXXX'}</span> নম্বরে ৬ ডিজিটের কোড পাঠানো হয়েছে।
                      </>
                    ) : (
                      <>
                        For account security, a 6-digit SMS verification code was dispatched to <span className="font-bold text-pink-400">{phoneOrEmail || '017XXXXXXXX'}</span>.
                      </>
                    )
                  ) : otpChannel === 'voice' ? (
                    language === 'bn' ? (
                      <>
                        ভয়েস কলের মাধ্যমে আপনার ওটিপি কোড পাঠ করা হচ্ছে। মনোযোগ দিয়ে শুনুন।
                      </>
                    ) : (
                      <>
                        Your OTP verification code is being read via voice call. Please listen carefully.
                      </>
                    )
                  ) : language === 'bn' ? (
                    <>
                      অ্যাকাউন্টের সুরক্ষায় আপনার <span className="font-bold text-pink-400 font-mono">{emailInput || phoneOrEmail || 'user@example.com'}</span> ইমেইলে কোড পাঠানো হয়েছে।
                    </>
                  ) : (
                    <>
                      For account security, a 6-digit email verification code was sent to <span className="font-bold text-pink-400">{emailInput || phoneOrEmail || 'user@example.com'}</span>.
                    </>
                  )}
                </p>
              </div>
              ...
              {isOtpSentBannerVisible && (
                <div className="p-3 bg-emerald-950/60 border border-emerald-500/40 rounded-2xl text-xs text-emerald-200 font-medium space-y-2 animate-in slide-in-from-top duration-300">
                  ...
                  {/* Live SMS vs Sandbox / Gateway Error Fallback Display */}
                  {generatedOtp && (
                    getStoredSmsConfig().isLiveMode && getStoredSmsConfig().apiKey && smsDispatchSuccess !== false && (otpChannel === 'sms' || otpChannel === 'voice') ? (
                      <div className="flex flex-col gap-2 bg-emerald-950/70 p-2.5 rounded-xl border border-emerald-500/40 text-emerald-200 text-xs font-semibold">
                        <div className="flex items-center justify-between gap-2">
                          <div className="flex items-center gap-2 min-w-0">
                            ...
                            <span className="truncate">
                              {otpChannel === 'voice'
                                ? 'ভয়েস কল ওটিপি চলছে। পুনরায় শুনতে বাটন চাপুন।'
                                : language === 'bn'
                                ? 'আপনার মোবাইলের ইনবক্স চেক করে ওটিপি কোডটি লিখুন।'
                                : 'Check your phone SMS inbox and enter the code.'}
                            </span>
                          </div>
                          <button
                            type="button"
                            onClick={() => playVoiceOtp(generatedOtp, getStoredSmsConfig().voiceTemplate)}
                            className="shrink-0 text-[10px] bg-purple-600 hover:bg-purple-500 text-white font-bold px-2.5 py-1 rounded-lg flex items-center gap-1 cursor-pointer shadow-xs"
                          >
                      ...
                      <div className="flex flex-col gap-2 bg-emerald-900/60 p-2.5 rounded-xl border border-emerald-500/40">
                        {smsDispatchSuccess === false && (
                          <div className="flex items-center justify-between text-[11px] text-amber-300 font-bold border-b border-white/10 pb-1.5">
                            <span>⚠️ SMS গেটওয়ে এরর (বিকল্প ওটিপি ব্যবহার করুন):</span>
                            <button
                              type="button"
                              onClick={() => playVoiceOtp(generatedOtp, getStoredSmsConfig().voiceTemplate)}
                              className="text-[10px] bg-purple-600 hover:bg-purple-500 text-white font-bold px-2 py-0.5 rounded flex items-center gap-1 cursor-pointer"
                            >
                              <Volume2 className="w-3 h-3" />
                              <span>🔊 ভয়েস ওটিপি</span>
                            </button>
                          </div>
                        )}
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-1.5">
                            <span className="text-[10px] text-emerald-300 font-bold uppercase bg-emerald-950 px-1.5 py-0.5 rounded border border-emerald-500/30">
                              {smsDispatchSuccess === false ? 'বিকল্প ওটিপি' : language === 'bn' ? 'টেস্ট মোড' : 'Test Sandbox'}
                            </span>
                            <span className="text-[11px] text-emerald-200 font-bold">
                              {language === 'bn' ? 'ওটিপি:' : 'Code:'}
                            </span>
              ...
              <form onSubmit={handleVerifyOtp} className="space-y-4">
                <div>
                  <div className="flex items-center justify-between mb-2 px-1">
                    <label className="block text-xs font-bold text-zinc-200">
                      {language === 'bn' ? '৬ ডিজিটের ওটিপি কোডটি লিখুন (মেয়াদ ৪ মিনিট):' : 'Type 6-Digit OTP (Valid 4 Mins):'}
                    </label>
                    <div className={`flex items-center gap-1.5 text-[11px] font-mono font-bold px-2.5 py-0.5 rounded-full border transition-all ${
                      resendTimer <= 30
                        ? 'text-red-300 bg-red-500/20 border-red-500/50 animate-pulse'
                        : 'text-pink-400 bg-pink-500/10 border-pink-500/30'
                    }`}>
                      <span className="animate-pulse">⏳</span>
                      <span>{formatTimer(resendTimer)}</span>
                      {resendTimer === 0 && (
                        <span className="text-[10px] text-red-400 font-sans font-bold">
                  ...
                  {/* 6-Digit Box Inputs matching the Login theme */}
                  <div className="flex items-center justify-center gap-2 sm:gap-3 my-3">
                    {[0, 1, 2, 3, 4, 5].map((index) => {
                      const rawChar = enteredOtp[index];
                      const digit = (rawChar && rawChar !== ' ') ? rawChar : '';
                      ...
                      return (
                        <input
                          ...
                          className={`w-11 h-12 sm:w-12 sm:h-14 rounded-2xl text-center text-xl sm:text-2xl font-mono font-bold transition-all duration-200 focus:outline-none select-all ${
                            isFilled
                              ? 'bg-purple-950/60 border-2 border-purple-500/80 text-white shadow-[0_0_12px_rgba(168,85,247,0.3)] font-black'
                              : 'bg-white/[0.06] border-2 border-white/15 text-white/90 hover:border-purple-500/50 hover:bg-white/[0.09] shadow-inner'
                          } focus:border-pink-500 focus:bg-pink-500/10 focus:ring-2 focus:ring-pink-500/30 focus:shadow-[0_0_20px_rgba(236,72,153,0.45)] focus:scale-[1.03]`}
                        />
                      );
                    })}
                  </div>
                ...
                >
                  ...
                  <span className="text-sm font-bold tracking-wide">
                    {mode === 'login'
                      ? (language === 'bn' ? 'ওটিপি ভেরিফাই করে লগইন করুন' : 'Verify OTP & Log In')
                      : mode === 'forgot'
                      ? (language === 'bn' ? 'পাসওয়ার্ড রিসেট সম্পন্ন করুন' : 'Reset Password')
                      : (language === 'bn' ? 'ভেরিফাই ও সম্পন্ন করুন' : 'Verify & Complete')}
                  </span>
                  <div className="w-7 h-7 rounded-full bg-white/20 backdrop-blur-sm flex items-center justify-center text-white group-hover:translate-x-0.5 transition-transform shrink-0">
