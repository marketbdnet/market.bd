...
export const UserProfileModal: React.FC<UserProfileModalProps> = ({ isOpen, onClose }) => {
  ...
  const activeAdsCount = myAds.filter(ad => ad.status === 'active' || ad.status === 'approved' || !ad.status).length;
  const pendingAdsCount = myAds.filter(ad => ad.status === 'pending' || (ad as any).status === 'under_review' || (ad as any).status === 'in_review').length;
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl sm:rounded-3xl shadow-2xl border border-slate-200 dark:border-slate-800 w-full max-w-2xl max-h-[92vh] flex flex-col overflow-hidden relative text-slate-900 dark:text-white">
        ...
        <div className="bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 text-white p-4 sm:p-5 flex items-center justify-between border-b border-slate-800 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-1.5 flex-wrap">
                <h2 className="text-sm sm:text-base font-black truncate max-w-[170px] sm:max-w-[260px]">
                  {currentUser.name}
                </h2>
          ...
          <div className="flex items-center gap-2 shrink-0">
            {/* Direct Instant Log Out Button in Profile Header */}
            <button
              onClick={() => {
                logout();
                onClose();
              }}
              className="bg-red-600 hover:bg-red-700 active:scale-95 text-white font-black text-xs px-3 py-1.5 rounded-xl flex items-center justify-center transition shadow-sm cursor-pointer border border-red-500/50"
        ...
        <div className="flex border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/90 text-xs font-bold shrink-0 overflow-x-auto">
          ...
          {/* 5. Direct Tab Logout Button */}
          <button
            onClick={() => {
              logout();
              onClose();
            }}
            className="py-3 px-3 text-center transition cursor-pointer border-b-2 border-transparent text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-950/30 flex items-center justify-center whitespace-nowrap font-black"
            title={language === 'bn' ? 'লগ আউট' : 'Log Out'}
          >
        ...
        <div className="p-4 sm:p-5 overflow-y-auto flex-1 space-y-4">
          ...
          {/* TAB 2: VIEW PROFILE OVERVIEW (Simplified Bikroy.com Style) */}
          {activeTab === 'profile' && (
            <div className="space-y-3.5">
              {/* Essential Card: User Avatar, Name, Phone & Location */}
              <div className="bg-slate-50 dark:bg-slate-800/80 p-4 rounded-2xl border border-slate-200 dark:border-slate-700 space-y-3.5">
                <div className="flex items-center gap-3.5">
                  <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-pink-500 to-rose-600 text-white font-black text-2xl flex items-center justify-center shadow-md shrink-0 ring-2 ring-pink-500/20">
                    {currentUser.name ? currentUser.name.charAt(0).toUpperCase() : 'U'}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5 flex-wrap">
                      ...
                      <span className="bg-emerald-500/15 text-emerald-700 dark:text-emerald-300 text-[10px] font-extrabold px-2 py-0.5 rounded-full border border-emerald-500/30 flex items-center gap-1">
                        <ShieldCheck className="w-3 h-3 text-emerald-600 dark:text-emerald-400" />
                        {language === 'bn' ? 'ভেরিফাইড' : 'Verified'}
                      </span>
                    </div>

                    <div className="flex flex-wrap items-center gap-y-1 gap-x-3 text-xs text-slate-600 dark:text-slate-300 mt-1">
                      {currentUser.phone && (
                        <span className="flex items-center gap-1 font-mono font-bold text-pink-600 dark:text-pink-400">
                      ...
                      <span className="flex items-center gap-1 font-medium text-slate-500 dark:text-slate-400">
                        ...
                        <span>{currentUser.location?.district || 'Dhaka'}, {currentUser.location?.division || 'Dhaka'}</span>
                      </span>
                    </div>
                  </div>
                </div>

                {/* Core Stats Overview (Total, Active, Pending, Sold) */}
                <div className="grid grid-cols-4 gap-2 pt-1">
                  <div className="bg-white dark:bg-slate-900 p-2 sm:p-2.5 rounded-xl border border-slate-200 dark:border-slate-700 text-center">
                    <span className="block text-xs sm:text-sm font-black text-slate-900 dark:text-white">{myAds.length}</span>
                    <span className="block text-[10px] font-bold text-slate-500">{language === 'bn' ? 'মোট এড' : 'Total'}</span>
                  </div>
                  <div className="bg-white dark:bg-slate-900 p-2 sm:p-2.5 rounded-xl border border-slate-200 dark:border-slate-700 text-center">
                    <span className="block text-xs sm:text-sm font-black text-emerald-600 dark:text-emerald-400">{activeAdsCount}</span>
                    <span className="block text-[10px] font-bold text-emerald-600 dark:text-emerald-400">{language === 'bn' ? 'একটিভ' : 'Active'}</span>
                  </div>
                  <div className="bg-white dark:bg-slate-900 p-2 sm:p-2.5 rounded-xl border border-slate-200 dark:border-slate-700 text-center">
                    <span className="block text-xs sm:text-sm font-black text-amber-600 dark:text-amber-400">{pendingAdsCount}</span>
                    <span className="block text-[10px] font-bold text-amber-600 dark:text-amber-400">{language === 'bn' ? 'পেন্ডিং' : 'Pending'}</span>
                  </div>
                  <div className="bg-white dark:bg-slate-900 p-2 sm:p-2.5 rounded-xl border border-slate-200 dark:border-slate-700 text-center">
                    <span className="block text-xs sm:text-sm font-black text-purple-600 dark:text-purple-400">{soldAdsCount}</span>
                    <span className="block text-[10px] font-bold text-purple-600 dark:text-purple-400">{language === 'bn' ? 'বিক্রি' : 'Sold'}</span>
                  </div>
                </div>

                {/* Primary Action Button to View Ads */}
                <button
                  onClick={() => setActiveTab('my-ads')}
                  className="w-full py-2.5 px-3 bg-pink-50 hover:bg-pink-100 dark:bg-pink-950/50 dark:hover:bg-pink-900/50 border border-pink-200 dark:border-pink-800 text-pink-700 dark:text-pink-300 font-extrabold rounded-xl text-xs transition flex items-center justify-between cursor-pointer"
                >
                  <span className="flex items-center gap-2">
                    <Package className="w-4 h-4 text-pink-600 dark:text-pink-400" />
                    <span>{language === 'bn' ? 'আমার সকল বিজ্ঞাপন পরিচালনা করুন' : 'Manage All My Ads'}</span>
                  </span>
                  <ChevronRight className="w-4 h-4" />
                </button>
              </div>

              {/* Collapsible "More" / "বিস্তারিত দেখুন" Button */}
              <button
                type="button"
                onClick={() => setShowMoreDetails(!showMoreDetails)}
                className="w-full py-2.5 px-4 bg-white dark:bg-slate-800/60 hover:bg-slate-50 dark:hover:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-xs font-extrabold text-slate-700 dark:text-slate-300 flex items-center justify-between transition shadow-2xs cursor-pointer"
              >
                <span className="flex items-center gap-2">
                  <span>⚙️</span>
                  <span>
                    {showMoreDetails
                      ? (language === 'bn' ? 'সংক্ষিপ্ত করুন (Show Less)' : 'Show Less')
                      : (language === 'bn' ? 'বিস্তারিত দেখুন ও সেটিংস (More Details & Settings)' : 'More Details & Settings')}
                  </span>
                </span>
                {showMoreDetails ? (
              ...
              {showMoreDetails && (
                <div className="space-y-3 animate-in fade-in slide-in-from-top-2 duration-200">
                  <div className="bg-slate-50 dark:bg-slate-800/80 p-4 rounded-2xl border border-slate-200 dark:border-slate-700 space-y-3 text-xs">
                    ...
                    <div className="flex items-center justify-between pb-2 border-b border-slate-200 dark:border-slate-700">
                      <span className="text-slate-500 font-bold">{language === 'bn' ? 'MarketBD.Net বয়স:' : 'MarketBD.Net History:'}</span>
                    ...
                    <div className="flex items-center justify-between">
                      <span className="text-slate-500 font-bold">{language === 'bn' ? 'অ্যাকাউন্ট ভেরিফিকেশন:' : 'Account Verification:'}</span>
                      <span className="font-extrabold text-emerald-600 dark:text-emerald-400 flex items-center gap-1">
                        <CheckCircle2 className="w-3.5 h-3.5" />
                        {language === 'bn' ? 'ফোন নম্বর ভেরিফাইড' : 'Phone Verified'}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      onClick={() => setActiveTab('delete')}
                      className="py-2.5 px-3 bg-red-50 hover:bg-red-100 dark:bg-red-950/40 text-red-600 dark:text-red-400 border border-red-200 dark:border-red-900 font-bold rounded-xl text-xs transition flex items-center gap-1.5 cursor-pointer"
                    >
                      <Trash2 className="w-3.5 h-3.5" />
        ...
        {/* Persistent Bottom Bar with 1-Click Log Out */}
        <div className="p-3 bg-slate-100/90 dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 flex items-center justify-between gap-3 text-xs shrink-0 select-none">
          <div className="flex items-center gap-2 text-slate-500 dark:text-slate-400 text-[11px] truncate">
            ...
            <span className="truncate">{currentUser.phone || currentUser.email || currentUser.name}</span>
          </div>
          <div className="flex items-center gap-2 shrink-0">
            <button
              onClick={() => {
              ...
              className="px-3.5 py-1.5 bg-red-600 hover:bg-red-700 text-white font-black text-xs rounded-xl transition flex items-center justify-center cursor-pointer shadow-xs active:scale-95"
              title={language === 'bn' ? 'লগ আউট' : 'Log Out'}
            >
              <span>{language === 'bn' ? 'লগ আউট' : 'Log Out'}</span>
            </button>
            <button
              onClick={onClose}
              className="px-3 py-1.5 bg-slate-200 dark:bg-slate-800 hover:bg-slate-300 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 font-bold rounded-xl transition cursor-pointer"
            >
