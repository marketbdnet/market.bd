...
import { Product, Language, Location, FilterState, ChatThread, AppNotification, ChatMessage, AppReleaseInfo, PaymentPartnerItem, PaymentAccountsConfig, SystemNoticeConfig, ClockSettings, Category, AdminActiveSession, AdminActivityLog, AdminActionType } from '../types';
import { INITIAL_PRODUCTS } from '../data/mockProducts';
import { VERIFIED_LIVE_COMMUNITY_ADS } from '../data/liveCommunityAds';
...
import { db, auth, safeFirestoreSetDoc, safeFirestoreDeleteDoc, onAuthStateChanged, signOut, isFirestoreQuotaExceeded } from '../lib/firebase';
import { doc, onSnapshot, collection, getDoc } from 'firebase/firestore';
import { INITIAL_ADMIN_LOGS, saveAdminLogToFirestore, subscribeToFirestoreAdminLogs, fetchAllAdminLogsFromFirestore, formatAdminLogTimestamp } from '../services/adminLogService';
...
interface MarketContextType {
  ...
  refreshAdminLogsFromCloud: () => Promise<void>;
  clearAdminLogs: () => void;
  spamThreads: string[];
...
export const MarketProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const normalizeProductsList = (productList: Product[]): Product[] => {
    return productList.map((p) => {
      ...
      return {
        ...
        postedAt: p.postedAt || '2026-09-01T12:00:00.000Z',
        createdAt: p.createdAt || p.postedAt || '2026-09-01T12:00:00.000Z',
        updatedAt: p.updatedAt || p.createdAt || p.postedAt || '2026-09-01T12:00:00.000Z',
  ...
  const [products, setProducts] = useState<Product[]>(() => {
    ...
    const oneYearLater = new Date(nowTime + 365 * 24 * 60 * 60 * 1000).toISOString();

    // Foundational 400 canonical listings - strictly preserved exactly as defined without dynamic mutations
    INITIAL_PRODUCTS.forEach((p) => {
      if (p && p.id) {
        map.set(p.id, {
          ...
          createdAt: p.createdAt || p.postedAt || '2026-09-01T12:00:00.000Z',
          updatedAt: p.updatedAt || p.createdAt || p.postedAt || '2026-09-01T12:00:00.000Z',
          expiresAt: p.expiresAt || oneYearLater
        });
      }
    });

    // Priority: Read v5 storage, fallback to v4 for seamless upgrade
    const saved = storage.getItem('marketbd_products_v5') || storage.getItem('marketbd_products_v4');
    if (saved) {
      try {
        const parsed = JSON.parse(saved);
        if (Array.isArray(parsed) && parsed.length > 0) {
          parsed.forEach((p: Product) => {
            if (p && p.id && p.title && p.title.trim() !== '' && !['prod-1787413284395', 'prod-pending-2', 'prod-pending-3', 'prod-1787183789329'].includes(p.id)) {
              const existing = map.get(p.id);
              const isPending = isProductPending(p);
              const isRejected = isProductRejected(p);
              const isSold = isProductSold(p);
              const effectiveStatus = isPending ? 'pending' : isRejected ? 'rejected' : isSold ? 'sold' : (p.status || 'active');

              const updatedP: Product = {
                ...p,
                status: effectiveStatus,
                isApproved: effectiveStatus === 'active' ? (p.isApproved ?? true) : false,
                isActive: effectiveStatus === 'active',
                moderationStatus: effectiveStatus === 'pending' ? 'pending' : effectiveStatus === 'rejected' ? 'rejected' : 'approved',
                postedAt: p.postedAt || existing?.postedAt || '2026-09-01T12:00:00.000Z',
                createdAt: p.createdAt || p.postedAt || existing?.createdAt || '2026-09-01T12:00:00.000Z',
                updatedAt: p.updatedAt || existing?.updatedAt || p.createdAt || p.postedAt || '2026-09-01T12:00:00.000Z',
                expiresAt: p.expiresAt || existing?.expiresAt || oneYearLater,
                expiryDate: undefined
              };

              if (existing) {
                map.set(p.id, { ...existing, ...updatedP });
              } else {
                map.set(p.id, updatedP);
              }
            }
          });
        }
    ...
    const finalNormalized = normalizeProductsList(Array.from(map.values()));
    // Defer writing heavy product cache to disk so the initial React render is instantaneous on mobile
    setTimeout(() => {
      try {
        storage.setItem('marketbd_products_v5', JSON.stringify(finalNormalized));
      } catch (e) {}
    }, 1200);
  ...
  const [registeredUsers, setRegisteredUsersState] = useState<UserProfile[]>(() => {
    try {
      const isCleaned = storage.getItem('marketbd_registered_users_cleanup_v2');
      if (!isCleaned) {
        storage.setItem('marketbd_registered_users_cleanup_v2', 'true');
        storage.setItem('marketbd_registered_users', JSON.stringify(INITIAL_REGISTERED_USERS));
  ...
  useEffect(() => {
    // 1. Initial hydration from server sync API (no-store ensures latest ads without browser caching)
    fetch('/api/sync/state?_t=' + Date.now(), { cache: 'no-store' })
      .then(res => res.json())
      .then(res => {
        if (res && res.success && res.data) {
          ...
          if (d.marketplace_products && Array.isArray(d.marketplace_products) && d.marketplace_products.length > 0) {
            setProducts(prev => {
              const map = new Map<string, Product>();
              INITIAL_PRODUCTS.forEach(p => { if (p && p.id) map.set(p.id, { ...p, status: 'active', isApproved: true, isActive: true }); });
              ...
              storage.setItem('marketbd_products_v5', JSON.stringify(merged));
              return merged;
            });
          }
    ...
    try {
      ...
      if (!isFirestoreQuotaExceeded() && !storage.getItem('marketbd_legacy_migrated_v1')) {
        storage.setItem('marketbd_legacy_migrated_v1', 'true');
      ...
      // Cross-platform sync: Ensure all 18 VERIFIED_LIVE_COMMUNITY_ADS are seeded in Firestore products collection (one-time only)
      if (!isFirestoreQuotaExceeded() && !storage.getItem('marketbd_live_comm_seeded_v1')) {
        storage.setItem('marketbd_live_comm_seeded_v1', 'true');
        VERIFIED_LIVE_COMMUNITY_ADS.forEach((commAd) => {
          if (commAd && commAd.id) {
            safeFirestoreSetDoc(doc(db, 'products', commAd.id), commAd, { merge: true }).catch(() => {});
          }
        });
      }
      ...
      const unsubProductsCol = onSnapshot(collection(db, 'products'), (snapshot) => {
        ...
        // Cross-Device Realtime Live Notification Trigger:
        // Detects when any ad posted from this device/account transitions to active/approved
        try {
          const myPostedAdsRaw = storage.getItem('marketbd_my_posted_ads');
          const myPostedAds: string[] = myPostedAdsRaw ? JSON.parse(myPostedAdsRaw) : [];
          
          cloudAds.forEach(ca => {
            const isMine = (
              myPostedAds.includes(ca.id) ||
              (currentUser?.email && ca.seller?.email === currentUser.email) ||
              (currentUser?.phone && ca.seller?.phone === currentUser.phone)
            );

            if (isMine && (ca.status === 'active' || ca.isApproved) && !storage.getItem('marketbd_notified_live_' + ca.id)) {
              storage.setItem('marketbd_notified_live_' + ca.id, 'true');
              const liveTitle = language === 'bn' ? '🎉 বিজ্ঞাপন এখন লাইভ!' : '🎉 Your advertisement is now live!';
              const liveMsg = language === 'bn' 
                ? `আপনার "${ca.title}" বিজ্ঞাপনটি সফলভাবে অনুমোদিত হয়েছে এবং ক্রেতাদের জন্য লাইভ প্রদর্শিত হচ্ছে।`
                : `Your advertisement "${ca.title}" has been approved and is now live.`;

              const notifId = 'notif-live-' + ca.id;
          ...
          console.warn('Realtime live notification check note:', e);
        }
        ...
        setProducts(prev => {
          ...
          prev.forEach(p => { if (p && p.id) map.set(p.id, p); });
          cloudAds.forEach(ca => {
            if (ca && ca.id && ca.title && ca.title.trim() !== '' && !['prod-1787413284395', 'prod-pending-2', 'prod-pending-3', 'prod-1787183789329'].includes(ca.id)) {
              const existing = map.get(ca.id);
              map.set(ca.id, existing ? { ...existing, ...ca } : ca);
            }
          });
          // Handle deletions: only delete non-canonical custom customer ads
          docChanges.forEach(change => {
            if (change.type === 'removed') {
              const isCanonical = INITIAL_PRODUCTS.some(ip => ip.id === change.doc.id);
              if (!isCanonical) {
      ...
      const unsubUsers = onSnapshot(doc(db, 'settings', 'registered_users'), (snapshot) => {
        if (snapshot.exists()) {
          ...
          if (Array.isArray(val)) {
            setRegisteredUsersState(val);
            storage.setItem('marketbd_registered_users', JSON.stringify(val));
          }
        }
      }, () => {});
      unsubs.push(unsubUsers);

      // Authoritative Realtime listener for cross-device User Notifications (e.g. ad approval on Web delivered to Android)
      const unsubUserNotifications = onSnapshot(collection(db, 'user_notifications'), (snapshot) => {
        try {
          const myPostedAdsRaw = storage.getItem('marketbd_my_posted_ads');
          const myPostedAds: string[] = myPostedAdsRaw ? JSON.parse(myPostedAdsRaw) : [];
          ...
          snapshot.docChanges().forEach(change => {
            if (change.type === 'added' || change.type === 'modified') {
              ...
              const isMine = (
                (data.targetProductId && myPostedAds.includes(data.targetProductId)) ||
                (currentUser?.email && data.sellerEmail === currentUser.email) ||
                (currentUser?.phone && data.sellerPhone === currentUser.phone)
              );

              if (isMine && !storage.getItem('marketbd_notif_seen_' + change.doc.id)) {
                storage.setItem('marketbd_notif_seen_' + change.doc.id, 'true');
                ...
                setNotifications(prev => {
                  ...
                  return [
                    {
                      id: change.doc.id,
                      title: data.title || 'বিজ্ঞাপন আপডেট',
                ...
                sendLocalBrowserPushNotification({
                  title: data.title || 'বিজ্ঞাপন আপডেট',
                  body: data.message || '',
                  url: data.targetProductId ? `#/product/${data.targetProductId}` : '/'
                });
              }
            }
          });
        } catch (err) {
          console.warn('User notifications listener note:', err);
        }
      }, () => {});
      unsubs.push(unsubUserNotifications);
      ...
      const unsubAdminSession = onSnapshot(doc(db, 'settings', 'admin_active_session'), (snapshot) => {
        if (snapshot.exists()) {
          ...
          if (val && val.sessionToken) {
            ...
            const adminSessionCreatedAt = Number(storage.getItem('marketbd_admin_session_created_at') || 0);

            // Ignore if in fresh login grace period (30s) or if snapshot has older/same timestamp
            const isFreshLogin = Date.now() - adminSessionCreatedAt < 30000;
            const remoteTime = new Date(val.loginTime || 0).getTime();
            const isOlderOrSame = remoteTime > 0 && adminSessionCreatedAt > 0 && remoteTime <= adminSessionCreatedAt;

            if (localUserRole === 'admin' && localAdminToken && val.sessionToken !== localAdminToken && !isFreshLogin && !isOlderOrSame) {
              ...
              storage.removeItem('marketbd_admin_session_created_at');
              setIsLoggedIn(false);
              setUserRole('buyer');
              setCurrentUser(null);
              setActiveTab('home');
      ...
      if (fbUser) {
        ...
        try {
          ...
          unsubUserDoc = onSnapshot(doc(db, 'users', fbUser.uid), (docSnap) => {
            ...
            const currentSavedRaw = storage.getItem('marketbd_auth_user');
            let currentSaved: UserProfile | null = null;
            if (currentSavedRaw) {
              try { currentSaved = JSON.parse(currentSavedRaw); } catch {}
            }
            ...
            if (docSnap.exists()) {
              ...
              profile = {
                id: fbUser.uid,
                name: data.name || fbUser.displayName || currentSaved?.name || fbUser.email?.split('@')[0] || 'MarketBD User',
                phone: data.phone || fbUser.phoneNumber || currentSaved?.phone || '',
                email: data.email || fbUser.email || currentSaved?.email || '',
                role: data.role || currentSaved?.role || 'seller',
                gender: data.gender || currentSaved?.gender || 'male',
                isVerified: data.isVerified ?? true,
                isBlocked: data.isBlocked ?? false,
                status: data.status || 'active',
                registeredAt: data.registeredAt || currentSaved?.registeredAt || fbUser.metadata.creationTime || new Date().toISOString(),
                avatar: data.avatar || fbUser.photoURL || currentSaved?.avatar,
                authProvider: data.authProvider || fbUser.providerData?.[0]?.providerId || 'firebase',
                location: data.location || currentSaved?.location,
                tradeLicense: data.tradeLicense,
                nidNumber: data.nidNumber,
                notes: data.notes,
                password: data.password || currentSaved?.password
              };
            } else {
              // Create default profile for this Firebase Auth user, preserving existing session data if available
              if (currentSaved && currentSaved.id) {
                profile = {
                  ...currentSaved,
                  id: currentSaved.id || fbUser.uid,
                  avatar: currentSaved.avatar || fbUser.photoURL || undefined,
                  authProvider: currentSaved.authProvider || fbUser.providerData?.[0]?.providerId || 'firebase'
                };
              } else {
                profile = {
                  id: fbUser.uid,
            ...
            if (err?.code !== 'cancelled') {
              console.warn('[Firebase Auth] Firestore user profile sync notice:', err?.message || err);
            }
            // Resilient Fallback: If Firestore snapshot fails or quota is exhausted, preserve the valid local user session
            const currentSavedRaw = storage.getItem('marketbd_auth_user');
            if (currentSavedRaw && storage.getItem('marketbd_is_logged_in') === 'true') {
              try {
                const currentSaved = JSON.parse(currentSavedRaw);
                if (currentSaved && !currentSaved.isBlocked && currentSaved.status !== 'blocked') {
                  setCurrentUser(currentSaved);
                  setIsLoggedIn(true);
                  setUserRole(currentSaved.role || 'seller');
                }
              } catch {}
            }
          });
  ...
  // Manual / Pull-to-Refresh listings updater
  const refreshProducts = async (): Promise<void> => {
    try {
      const res = await fetch('/api/products');
      if (res.ok) {
        const data = await res.json();
        const productArray = Array.isArray(data.data) ? data.data : (Array.isArray(data.products) ? data.products : (Array.isArray(data) ? data : null));
        if (productArray && productArray.length > 0) {
          const map = new Map<string, Product>();
          INITIAL_PRODUCTS.forEach(p => { if (p && p.id) map.set(p.id, { ...p, status: 'active', isApproved: true, isActive: true }); });
          productArray.forEach((p: Product) => {
            if (p && p.id && p.title && p.title.trim() !== '' && !['prod-1787413284395', 'prod-pending-2', 'prod-pending-3', 'prod-1787183789329'].includes(p.id)) {
              const existing = map.get(p.id);
              map.set(p.id, existing ? { ...existing, ...p } : p);
            }
          });
          const normalized = normalizeProductsList(Array.from(map.values()));
          setProducts(normalized);
          storage.setItem('marketbd_products_v4', JSON.stringify(normalized));
        }
      }
    } catch (e) {
      console.warn('Error manual refreshing products:', e);
    }
  };

  // Cache products to client Local Storage (authoritative state remains on server & database)
  // CRITICAL: Automatic bulk client sync to /api/products/sync is permanently disabled to prevent 
  // browser or WebView caches from overwriting the server's authoritative product catalog.
  useEffect(() => {
    try {
      storage.setItem('marketbd_products_v4', JSON.stringify(products));
    } catch (e) {
      console.error('Error saving products to local storage cache:', e);
    }
  }, [products]);
  ...
  // Admin Activity & Audit Logs (Real-time Firestore persistence & multi-device sync)
  const [adminActivityLogs, setAdminActivityLogs] = useState<AdminActivityLog[]>(() => {
    try {
      const saved = storage.getItem('marketbd_admin_activity_logs');
      if (saved) {
        const parsed = JSON.parse(saved);
        if (Array.isArray(parsed) && parsed.length > 0) {
          return parsed;
      ...
      console.error('Error reading stored admin activity logs:', e);
    }
    return INITIAL_ADMIN_LOGS;
  });

  // Real-time Firestore sync listener for admin_logs collection
  useEffect(() => {
    const unsubscribe = subscribeToFirestoreAdminLogs((remoteLogs) => {
      if (remoteLogs && remoteLogs.length > 0) {
        setAdminActivityLogs(prev => {
          ...
          try {
            storage.setItem('marketbd_admin_activity_logs', JSON.stringify(merged));
          } catch (e) {}
          return merged;
        });
      }
  ...
  const refreshAdminLogsFromCloud = async () => {
    try {
      const freshLogs = await fetchAllAdminLogsFromFirestore();
      if (freshLogs && freshLogs.length > 0) {
        setAdminActivityLogs(prev => {
          const map = new Map<string, AdminActivityLog>();
          freshLogs.forEach(l => map.set(l.id, l));
          prev.forEach(l => {
            if (!map.has(l.id)) map.set(l.id, l);
          });
          const merged = Array.from(map.values()).sort((a, b) => {
  ...
  const clearAdminLogs = () => {
    ...
    try {
      storage.removeItem('marketbd_admin_activity_logs');
    } catch (e) {}
  };

  const logAdminAction = (entry: {
    action: string;
    actionType: AdminActionType;
    targetId?: string;
    targetTitle?: string;
    ...
    const device = isMobile ? 'Android / Mobile App' : 'Desktop / PC Browser';
    const ip = '103.110.22.4';

    const newLog: AdminActivityLog = {
      id: 'adm-log-' + Date.now() + '-' + Math.random().toString(36).substring(2, 7),
      adminId: currentUser?.id || 'admin-master',
      ...
      timestamp: formatAdminLogTimestamp(),
    ...
    setAdminActivityLogs(prev => {
      ...
      try {
        storage.setItem('marketbd_admin_activity_logs', JSON.stringify(updated));
      } catch (e) {}
      return updated;
    });

    // Write to Firestore collection 'admin_logs'
    saveAdminLogToFirestore(newLog).catch(() => {});

    // Sync to REST API for multi-platform sync
    try {
      fetch('/api/sync/state', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ latestAdminLog: newLog })
      }).catch(() => {});
    } catch (e) {}
  };
  ...
  const deleteProductWithReason = (productId: string, reason: string, customNote?: string) => {
    ...
    // 🛡️ Log Admin Action in Firestore
    logAdminAction({
      action: 'Deleted Ad',
      actionType: 'ad_moderation',
      targetId: productId,
      targetTitle: deletedTitle,
      details: `Ad permanently removed from marketplace. Reason: ${reason}${customNote ? ` (${customNote})` : ''}`,
      metadata: { productId, reason, customNote }
    });
  };
  ...
  const [userRole, setUserRole] = useState<'buyer' | 'seller' | 'admin'>(() => {
    const savedRole = storage.getItem('marketbd_user_role') as any;
    const savedUser = storage.getItem('marketbd_auth_user');
    if (savedRole === 'admin' && savedUser) {
      try {
        const parsed = JSON.parse(savedUser);
        if (parsed && parsed.id !== 'admin-master') {
          return 'seller';
        }
      } catch (e) {}
    }
  ...
  const [currentUser, setCurrentUser] = useState<UserProfile | null>(() => {
    ...
    if (saved) {
      try {
        const parsed = JSON.parse(saved);
        if (parsed && parsed.id !== 'admin-master' && parsed.role === 'admin') {
          parsed.role = 'seller';
          storage.setItem('marketbd_auth_user', JSON.stringify(parsed));
  ...
  const toggleBlockUser = (userId: string, reason?: string) => {
    setRegisteredUsersState(prev => {
      ...
      // 🛡️ Log Admin Action in Firestore
      const targetUser = updated.find(u => u.id === userId || u.phone === userId || u.email === userId);
      const isNowBlocked = targetUser?.isBlocked;
      logAdminAction({
        action: isNowBlocked ? 'Blocked User' : 'Unblocked User',
        actionType: 'user_management',
        targetId: userId,
        targetTitle: targetUser?.name || userId,
        details: isNowBlocked
          ? `User account suspended and blocked. Reason: ${reason || 'Admin policy suspension'}`
          : 'User account unblocked and restored to active state.'
      });
  ...
  const deleteUserById = (userId: string) => {
    setRegisteredUsersState(prev => {
      const targetUser = prev.find(u => u.id === userId || u.phone === userId || u.email === userId);
      ...
      logAdminAction({
        ...
        targetId: userId,
        targetTitle: targetUser?.name || userId,
        details: `Customer account (${targetUser?.name || 'N/A'}, Phone: ${targetUser?.phone || 'N/A'}) permanently removed.`
      });
  ...
  const toggleVerifyUser = (userId: string) => {
    setRegisteredUsersState(prev => {
      ...
      const updated = prev.map(u => {
        if (u.id === userId || u.phone === userId || u.email === userId) {
          isVerifiedNow = !u.isVerified;
          targetName = u.name;
          return { ...u, isVerified: isVerifiedNow };
        }
        return u;
      });
      storage.setItem('marketbd_registered_users', JSON.stringify(updated));
      ...
      // 🛡️ Log Admin Action in Firestore
      logAdminAction({
        action: isVerifiedNow ? 'Verified Merchant' : 'Revoked Verification',
        actionType: 'user_management',
        targetId: userId,
        targetTitle: targetName,
        details: isVerifiedNow
          ? 'Merchant verification gold badge approved and activated.'
          : 'Merchant verification badge removed.'
      });
  ...
  const addManualUser = (userData: Partial<UserProfile>) => {
    ...
    logAdminAction({
      ...
      details: `New ${newUser.role} user created manually (Phone: ${newUser.phone}, Location: ${newUser.location?.district || 'Dhaka'}).`
    });
  };
  ...
  const updateRegisteredUser = (userId: string, updates: Partial<UserProfile>) => {
    ...
    // 🛡️ Log Admin Action in Firestore
    logAdminAction({
      action: updates.role ? 'Changed User Role' : 'Updated User Profile',
      actionType: 'user_management',
      targetId: userId,
      targetTitle: targetUserName,
      details: updates.role
        ? `User role updated to "${updates.role.toUpperCase()}".`
        : `Customer account details updated: ${Object.keys(updates).join(', ')}.`
    });
  };
  ...
  const login = (userData?: Partial<UserProfile>) => {
    ...
    const profile: UserProfile = {
      ...
      email: userData?.email !== undefined ? userData.email : (role === 'admin' ? 'official.marketbd@gmail.com' : ''),
      role: role,
      gender: userData?.gender || 'male',
      isVerified: userData?.isVerified ?? true,
    ...
    // Save persistent login session (Persists until explicit logout or superseding session)
    storage.setItem('marketbd_is_logged_in', 'true');
    storage.setItem('marketbd_user_role', role);
    storage.setItem('marketbd_auth_user', JSON.stringify(profile));

    // 🛡️ Universal Single-Device Concurrency: Create & Bind Unique Session Token for ANY logged-in account
    const nowMs = Date.now();
    const userSessionToken = 'usr_sess_' + profile.id + '_' + nowMs + '_' + Math.random().toString(36).substring(2, 9);
    storage.setItem('marketbd_current_session_token', userSessionToken);
    storage.setItem('marketbd_user_session_token_' + profile.id, userSessionToken);
    storage.setItem('marketbd_session_created_at', String(nowMs));

    const userAgent = typeof navigator !== 'undefined' ? navigator.userAgent : '';
    const isMobile = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(userAgent);
    const userDeviceName = isMobile ? 'Mobile / Android Device' : 'Desktop / PC Browser';

    const userSessionPayload = {
      userId: profile.id,
      sessionToken: userSessionToken,
      userIdentifier: profile.phone || profile.email || profile.name,
    ...
    safeFirestoreSetDoc(doc(db, 'user_sessions', profile.id), userSessionPayload, { merge: true }).catch(() => {});
    fetch('/api/user/session', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(userSessionPayload)
    }).catch(() => {});

    // If Super Admin logs in, assign a single-device session token and announce to server/cloud
    if (role === 'admin' || (profile.role === 'admin' && profile.id === 'admin-master')) {
      const newSessionToken = 'adm_sess_' + nowMs + '_' + Math.random().toString(36).substring(2, 9);
      storage.setItem('marketbd_admin_session_token', newSessionToken);
      storage.setItem('marketbd_admin_session_created_at', String(nowMs));
  ...
  const logout = () => {
    ...
    if (currentUser?.id) {
      ...
      fetch(`/api/user/session/${uid}`, { method: 'DELETE' }).catch(() => {});
      safeFirestoreSetDoc(doc(db, 'user_sessions', uid), { sessionToken: 'logged_out', updatedAt: new Date().toISOString() }, { merge: true }).catch(() => {});
      storage.removeItem('marketbd_user_session_token_' + uid);
    }
    storage.removeItem('marketbd_current_session_token');
    storage.removeItem('marketbd_admin_session_token');
  ...
  // Universal Single-Device Concurrency Enforcement Monitor (All Logged-in Accounts)
  useEffect(() => {
    if (!isLoggedIn || !currentUser?.id) return;

    const currentUserId = currentUser.id;
    let localToken = storage.getItem('marketbd_current_session_token') || storage.getItem('marketbd_user_session_token_' + currentUserId);

    if (!localToken) {
      localToken = 'usr_sess_' + currentUserId + '_' + Date.now() + '_' + Math.random().toString(36).substring(2, 9);
      storage.setItem('marketbd_current_session_token', localToken);
      storage.setItem('marketbd_user_session_token_' + currentUserId, localToken);
      const isMobile = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent || '');
      const payload = {
        userId: currentUserId,
        sessionToken: localToken,
        userIdentifier: currentUser.phone || currentUser.email || currentUser.name,
        deviceName: isMobile ? 'Mobile / Android Device' : 'Desktop / PC Browser',
        loginTime: new Date().toISOString()
      };
      safeFirestoreSetDoc(doc(db, 'user_sessions', currentUserId), payload, { merge: true }).catch(() => {});
      fetch('/api/user/session', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    // 1. Realtime Firestore listener for instant auto-logout
    const unsubSession = onSnapshot(doc(db, 'user_sessions', currentUserId), (snapshot) => {
      if (snapshot.exists()) {
        // Skip check if this snapshot contains our own local pending writes
        if (snapshot.metadata.hasPendingWrites) return;

        // If Firestore quota is exceeded, remote docs are stale; do not trigger false-positive logout
        if (isFirestoreQuotaExceeded()) return;

        const data = snapshot.data();
        const activeToken = storage.getItem('marketbd_current_session_token') || storage.getItem('marketbd_user_session_token_' + currentUserId);
        const sessionCreatedAt = Number(storage.getItem('marketbd_session_created_at') || 0);

        // Ignore if within fresh login grace period (45 seconds)
        const isFreshLogin = Date.now() - sessionCreatedAt < 45000;
        if (isFreshLogin) return;

        // Verify remote session is strictly newer than when this device logged in
        const remoteTime = new Date(data?.updatedAt || data?.loginTime || 0).getTime();
        const isRemoteNewer = remoteTime > 0 && sessionCreatedAt > 0 && remoteTime > (sessionCreatedAt + 3000);

        if (data && data.sessionToken && activeToken && data.sessionToken !== activeToken && data.sessionToken !== 'logged_out' && isRemoteNewer) {
          console.warn('[Security] Account logged in from another device. Auto-logging out this device.', data);
          logout();
          alert(
            language === 'bn'
              ? `⚠️ **একক ডিভাইস সিকিউরিটি অ্যালার্ট**\n\nআপনার এই অ্যাকাউন্টটি অন্য একটি নতুন ডিভাইস (${data.deviceName || 'নতুন ডিভাইস'}) থেকে লগ ইন করা হয়েছে।\nআমাদের সিঙ্গেল-ডিভাইস পলিসি অনুযায়ী একটি অ্যাকাউন্ট থেকে একই সাথে কেবল একটি ডিভাইসেই লগ ইন থাকা সম্ভব।\nতাই এই ডিভাইস থেকে স্বয়ংক্রিয়ভাবে লগ আউট করা হলো।`
              : `⚠️ **Single-Device Security Alert**\n\nYour account was logged into from another device (${data.deviceName || 'New Device'}).\nOnly one active session per account is permitted. You have been automatically logged out on this device.`
          );
        }
      }
    }, () => {});

    // 2. Redundant check via server session API
    const checkServerSession = () => {
      fetch(`/api/user/session/${currentUserId}`)
        .then(r => r.json())
        .then(r => {
          if (r?.success && r?.data) {
            const remote = r.data;
            ...
            // Only trigger if remote session is strictly newer than when this device logged in
            const remoteTime = new Date(remote?.updatedAt || remote?.loginTime || 0).getTime();
            const isRemoteNewer = remoteTime > 0 && sessionCreatedAt > 0 && remoteTime > (sessionCreatedAt + 3000);

            if (remote.sessionToken && activeToken && remote.sessionToken !== activeToken && remote.sessionToken !== 'logged_out' && isRemoteNewer) {
              console.warn('[Security] Detected superseding session from server API.', remote);
              logout();
              alert(
                language === 'bn'
                  ? `⚠️ **একক ডিভাইস সিকিউরিটি অ্যালার্ট**\n\nআপনার এই অ্যাকাউন্টটি অন্য একটি নতুন ডিভাইস (${remote.deviceName || 'নতুন ডিভাইস'}) থেকে লগ ইন করা হয়েছে।\nআমাদের সিঙ্গেল-ডিভাইস পলিসি অনুযায়ী একটি অ্যাকাউন্ট থেকে একই সাথে কেবল একটি ডিভাইসেই লগ ইন থাকা সম্ভব।\nতাই এই ডিভাইস থেকে স্বয়ংক্রিয়ভাবে লগ আউট করা হলো।`
                  : `⚠️ **Single-Device Security Alert**\n\nYour account was logged into from another device (${remote.deviceName || 'New Device'}).\nOnly one active session per account is permitted. You have been automatically logged out on this device.`
              );
            }
          }
        })
    ...
    const interval = setInterval(checkServerSession, 6000);

    return () => {
      unsubSession();
      clearInterval(interval);
    };
  }, [isLoggedIn, currentUser?.id, language]);

  // Super Admin Single-Device Concurrency Enforcement Monitor
  useEffect(() => {
    const isCurrentUserAdmin = userRole === 'admin' || currentUser?.role === 'admin';
    ...
    const checkAdminSession = () => {
      ...
      const adminSessionCreatedAt = Number(storage.getItem('marketbd_admin_session_created_at') || 0);
      const isFreshLogin = Date.now() - adminSessionCreatedAt < 45000;
      if (isFreshLogin) return;
      ...
      fetch('/api/admin/session')
        ...
        .then(res => {
          if (res && res.success && res.data) {
            ...
            const remoteTime = new Date(remote.loginTime || 0).getTime();
            const isRemoteNewer = remoteTime > 0 && adminSessionCreatedAt > 0 && remoteTime > (adminSessionCreatedAt + 3000);

            setAdminActiveSession(remote);
            storage.setItem('marketbd_admin_active_session', JSON.stringify(remote));

            if (remote.sessionToken && remote.sessionToken !== localToken && isRemoteNewer) {
    ...
    const interval = setInterval(checkAdminSession, 6000);
    return () => clearInterval(interval);
  ...
  const sendMessage = (threadId: string, text: string, offerAmount?: number, imageUrl?: string) => {
    ...
    // 1. Mark as delivered after short realistic delay
    setTimeout(() => {
  ...
  const openChatForProduct = (product: Product): string => {
    ...
    // Trigger delivery status after a short network delay
    setTimeout(() => updateMessageStatus(threadId, initMsgId, 'delivered'), 600);
  ...
  const addNewAd = (adData: Partial<Product>): Product => {
    ...
    const newProduct: Product = makeApprovedProduct(rawProduct as Product);
    ...
    try {
      const myPostedAdsRaw = storage.getItem('marketbd_my_posted_ads');
      const myPostedAds: string[] = myPostedAdsRaw ? JSON.parse(myPostedAdsRaw) : [];
      if (!myPostedAds.includes(newProduct.id)) {
        myPostedAds.push(newProduct.id);
        storage.setItem('marketbd_my_posted_ads', JSON.stringify(myPostedAds));
      }
    } catch (e) {}
    ...
    const newAdNotifMsg = language === 'bn'
      ? `আপনার "${newProduct.title}" বিজ্ঞাপনটি পর্যালোচনার জন্য জমা হয়েছে। অ্যাডমিন অনুমোদন করলেই লাইভ হবে।`
      : `Your advertisement "${newProduct.title}" is submitted for review. It will go live once approved.`;
    ...
    try {
      const notifDoc = {
        ...
        sellerPhone: newProduct.seller?.phone || '',
        sellerEmail: newProduct.seller?.email || '',
        type: 'approval',
        isRead: false
      };
      safeFirestoreSetDoc(doc(db, 'user_notifications', newAdReviewId), notifDoc, { merge: true }).catch(() => {});
    } catch (e) {}
  ...
  const updateProductStatus = (id: string, status: 'active' | 'approved' | 'pending' | 'sold' | 'rejected', reason?: string) => {
    ...
    if (effectiveStatus === 'active' && shouldSendLiveNotif && approvedLiveProduct) {
      ...
      sendLocalBrowserPushNotification({ title, body: message, url: `#/product/${approvedLiveProduct.id}` });
      ...
      try {
        const notifDoc = {
          ...
          message,
          time: new Date().toISOString(),
          targetProductId: approvedLiveProduct.id,
          sellerPhone: approvedLiveProduct.seller?.phone || '',
          sellerEmail: approvedLiveProduct.seller?.email || '',
          type: 'approval',
          isRead: false
        };
        safeFirestoreSetDoc(doc(db, 'user_notifications', notifId), notifDoc, { merge: true }).catch(() => {});
      } catch (e) {}
