...
import { KNOWN_JUNK_PRODUCT_IDS } from '../utils/knownJunkProductIds';
...
export const DEFAULT_SITE_MAINTENANCE: SiteMaintenanceConfig = {
  ...
  masterUnlockPin: '', // SECURITY: no default PIN — admin must set one explicitly in settings
  contactEmail: 'official.marketbd@gmail.com',
...
export const MarketProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [products, setProducts] = useState<Product[]>(() => {
    ...
    setTimeout(() => {
      try {
        storage.setItem('marketbd_products_v4', JSON.stringify(finalNormalized));
      } catch (e) {}
    }, 1200);
  ...
  useEffect(() => {
      ...
      .then(res => {
        if (res && res.success && res.data) {
          ...
          if (d.marketplace_products && Array.isArray(d.marketplace_products) && d.marketplace_products.length > 0) {
            setProducts(prev => {
              // Same fix as the main products-collection listener: don't
              // carry forward stale prev/local state, or deleted items can
              // never disappear for a browser that already cached them.
              const map = new Map<string, Product>();
    ...
    try {
      ...
      const unsubProductsCol = onSnapshot(collection(db, 'products'), (snapshot) => {
        ...
        setProducts(prev => {
          // FIX: previously this also merged in `prev` (the browser's own
          // last React state, which could still be carrying stale entries
          // from localStorage or an earlier session). That meant a product
          // deleted from Firestore could keep "ghost" living forever in a
          // browser/device that had already cached it locally -- which is
          // exactly why different browsers/devices could show different ad
          // counts even with this realtime listener running. Firestore
          // (cloudAds) plus the baked-in canonical catalog are now the only
          // two sources of truth, so every client converges on the same
          // Firestore document, live -- matching WEBSITE POST + ANDROID
          // POST -> same Firestore doc -> WEBSITE LIVE + ANDROID LIVE.
          const map = new Map<string, Product>();
          INITIAL_PRODUCTS.forEach(p => { if (p && p.id) map.set(p.id, { ...p, status: 'active', isApproved: true, isActive: true }); });
          cloudAds.forEach(ca => {
            if (ca && ca.id && ca.title && ca.title.trim() !== '' && !KNOWN_JUNK_PRODUCT_IDS.includes(ca.id)) {
              const existing = map.get(ca.id);
              map.set(ca.id, existing ? { ...existing, ...ca } : ca);
            }
          });
          // Any canonical/seed product that was deliberately removed from
          // Firestore (e.g. admin deleted it) should also disappear, not
          // just custom ads -- so drop the special-case exemption here too.
  ...
  // NOTE: an automatic call to refreshProducts() (which reads the legacy
  // Express /api/products file-cache, server_sync_state.json) used to run
  // here on every app start. Now that the Firestore onSnapshot listener
  // above is fixed to be self-healing (see the products collection
  // listener), Firestore is the single real-time source of truth for both
  // web and Android, per the requested architecture: both post to the same
  // Firestore doc, admin approves it once, and every client sees the same
  // doc live. Auto-calling the separate Express cache here would risk
  // overwriting the correct Firestore-driven state with a stale, separately
  // -updated snapshot, so it's been removed. The manual "pull to refresh"
  // button still calls refreshProducts() and remains harmless as a manual
  // fallback.
  ...
  const [chatThreads, setChatThreads] = useState<ChatThread[]>(() => {
    ...
    // NOTE: previously seeded a fake demo conversation here using
    // INITIAL_PRODUCTS[0]'s seller ('Rahim Tech Gadgets') so every single
    // user who opened Messages saw a stranger's fake chat. Removed -- a new
    // user with no real conversations should simply see an empty inbox.
    return [];
  });
  ...
  // Tracks which threads already fired a "new message" notification since
  // they were last opened/viewed, so a customer sending 5 messages in a row
  // triggers exactly ONE notification, not five. Cleared once the thread is
  // opened again, so a genuinely new round of messages notifies once more.
  const [notifiedThreadIds, setNotifiedThreadIds] = useState<Set<string>>(new Set());

  useEffect(() => {
    if (activeChat) {
      setNotifiedThreadIds(prev => {
        if (!prev.has(activeChat.id)) return prev;
  ...
  const sendMessage = (threadId: string, text: string, offerAmount?: number, imageUrl?: string) => {
    ...
    // 🔔 Seller Notification: fire at most ONCE per thread per unread batch —
    // not once for every message a customer sends. Re-enabled automatically
    // once the thread is opened/viewed again (see notifiedThreadIds effect).
    if (!notifiedThreadIds.has(threadId)) {
      setNotifiedThreadIds(prev => new Set(prev).add(threadId));
  ...
  const openChatForProduct = (product: Product): string => {
    // Messaging a seller now requires being logged in.
    if (!isLoggedIn) {
      openAuthModal('chat');
      return '';
    }
    ...
    // 🔔 Seller Notification for the first message of a brand-new conversation.
    setNotifiedThreadIds(prev => new Set(prev).add(threadId));
    const notifId = 'notif-msg-' + Date.now();
    const sellerNotifTitle = language === 'bn' ? 'একজন ক্রেতা আপনাকে মেসেজ করেছেন।' : 'You have a new message from a customer.';
    const sellerNotifMsg = language === 'bn'
  ...
  const addNewAd = (adData: Partial<Product>): Product => {
    ...
    // FIX: new ads were previously force-set to status:'active' /
    // isApproved:true / moderationStatus:'approved' right here, so every ad
    // (free or paid) went live instantly and admin approval was never
    // actually required. Now every new ad starts pending — the existing
    // auto-approval timer / admin panel is what promotes it to active.
    const newProduct: Product = makePendingProduct(rawProduct as Product);
  ...
  const unlockMasterLockdown = (enteredPin: string): boolean => {
    // SECURITY FIX: previously fell back to '7860' and also accepted the
    // hardcoded strings 'marketbdadmin' and '123456' as universal bypass
    // codes, regardless of what PIN the admin had actually configured. Those
    // were visible to anyone in the shipped JS bundle. Now only the PIN the
    // admin explicitly set in site settings is accepted. Note: this check
    // still runs client-side, so treat it as a UI gate, not real security —
    // for real protection, verify the PIN in a server/Cloud Function call
    // instead of comparing it in the browser.
    const expectedPin = siteMaintenance.masterUnlockPin;
