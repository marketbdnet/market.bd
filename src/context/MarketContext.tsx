...
export const MarketProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const normalizeProductsList = (productList: Product[]): Product[] => {
    ...
    return productList.map((p, index) => {
      let postedAt = p.postedAt;
      const parsedTime = postedAt ? new Date(postedAt).getTime() : NaN;
      if (isNaN(parsedTime) || parsedTime < now - 30 * 24 * 60 * 60 * 1000) {
        postedAt = new Date(now - (index * 25 + 5) * 60 * 1000).toISOString();
      }
      return {
        ...p,
        postedAt,
  ...
  const [products, setProducts] = useState<Product[]>(() => {
    ...
    return normalizeProductsList(INITIAL_PRODUCTS);
  });
      ...
      const unsubProducts = onSnapshot(doc(db, 'settings', 'marketplace_products'), (snapshot) => {
        if (snapshot.exists()) {
          ...
          if (Array.isArray(val) && val.length > 0) {
            setProducts(prev => {
              // Ensure we don't overwrite if lengths and items match
              if (JSON.stringify(prev) === JSON.stringify(val)) return prev;
              storage.setItem('marketbd_products_v4', JSON.stringify(val));
              return val;
            });
          }
        }
      ...
      const unsubProductsCol = onSnapshot(collection(db, 'products'), (snapshot) => {
        ...
        if (cloudAds.length > 0) {
          setProducts(prev => {
            ...
            cloudAds.forEach(ca => map.set(ca.id, ca));
            const merged = Array.from(map.values()).sort((a, b) => 
              new Date(b.postedAt || 0).getTime() - new Date(a.postedAt || 0).getTime()
            );
  ...
  useEffect(() => {
    ...
    const unsubAuth = onAuthStateChanged(auth, async (fbUser) => {
      ...
      if (fbUser) {
        ...
        // Check if user is logged in as Super Admin with offline token or manual bypass
        const savedAuth = storage.getItem('marketbd_auth_user');
        if (savedAuth) {
          try {
            const parsed = JSON.parse(savedAuth);
            if (parsed && (parsed.id === 'admin-master' || parsed.role === 'admin' || parsed.authProvider === 'manual')) {
              return;
            }
          } catch (e) {}
        }

        const wasLoggedIn = storage.getItem('marketbd_is_logged_in') === 'true';
        if (wasLoggedIn) {
          const savedRole = storage.getItem('marketbd_user_role');
          if (savedRole !== 'admin') {
            setIsLoggedIn(false);
            setUserRole('buyer');
            setCurrentUser(null);
      ...
      setProducts(prevProducts => {
          ...
          if (product.status === 'pending') {
            ...
            if (elapsed >= AUTO_APPROVE_MS) {
              ...
              autoApprovedNotifs.push({
                id: 'notif-autoapp-' + Date.now() + Math.random().toString(36).substring(2, 5),
                title: '⚡ বিজ্ঞাপন ৩০ মিনিট পর অটো-এপ্রুভ হয়েছে!',
                message: `আপনার "${product.title}" বিজ্ঞাপনটি এডমিন কর্তৃক নির্ধারিত ৩০ মিনিট পর্যালোচনার সময় অতিক্রান্ত হওয়ায় সিস্টেম দ্বারা স্বয়ংক্রিয়ভাবে (Auto-Approved) এপ্রুভ ও লাইভ করা হয়েছে!`,
                time: 'Just now',
                isRead: false,
                type: 'approval'
              });

              const approvedItem: Product = {
                ...product,
        ...
        if (hasChanges) {
          if (autoApprovedNotifs.length > 0) {
            setNotifications(prev => [...autoApprovedNotifs, ...prev]);
          ...
          try {
            ...
            newlyApprovedProducts.forEach(p => {
              safeFirestoreSetDoc(doc(db, 'products', p.id), { status: 'active', postedAt: p.postedAt }, { merge: true }).catch(() => {});
              fetch('/api/products/' + p.id, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ status: 'active', postedAt: p.postedAt })
              }).catch(() => {});
            });
          } catch (e) {
    ...
    setProducts(prevProducts => {
      ...
      const updated = prevProducts.map(product => {
        if (product.status === 'pending' && (!productId || product.id === productId)) {
          autoApprovedNotifs.push({
            id: 'notif-force-autoapp-' + Date.now() + Math.random().toString(36).substring(2, 5),
            title: '⚡ ৩০ মিনিট সমতুল্য অটো-এপ্রুভ সিস্টেম চালিত হয়েছে!',
            message: `আপনার "${product.title}" বিজ্ঞাপনটি ৩০ মিনিট পর স্বয়ংক্রিয়ভাবে (Auto-Approved) এপ্রুভ হয়ে প্ল্যাটফর্মে প্রকাশ করা হয়েছে!`,
            time: 'Just now',
            isRead: false,
            type: 'approval'
          });
  ...
  const addNewAd = (adData: Partial<Product>): Product => {
    const newProduct: Product = {
      ...
      seller: {
        ...
        phone: currentUser?.phone || adData.seller?.phone || '01634025151',
        hidePhone: Boolean(adData.seller?.hidePhone),
      ...
      status: 'pending' // Under Review status - waits for Admin approval or 30-min auto-approval
    };
    ...
    const newAdNotifTitle = language === 'bn' ? 'বিজ্ঞাপন আন্ডার রিভিউতে রয়েছে (Under Review) ⏳' : 'Ad Submitted for Review ⏳';
    const newAdNotifMsg = language === 'bn'
      ? `আপনার "${newProduct.title}" বিজ্ঞাপনটি সফলভাবে জমা হয়েছে এবং বর্তমানে আন্ডার রিভিউতে রয়েছে। এডমিন এটি পর্যালোচনা করবেন অথবা আগামী ৩০ মিনিটের মধ্যে কোনো সিদ্ধান্ত না নেওয়া হলে সিস্টেম দ্বারা এটি স্বয়ংক্রিয়ভাবে (Auto-Approved) এপ্রুভ ও লাইভ হয়ে যাবে!`
      : `Your ad "${newProduct.title}" has been submitted for review. If unreviewed by admin within 30 minutes, it will be automatically approved and published!`;
  ...
  const updateProductStatus = (id: string, status: 'active' | 'pending' | 'sold' | 'rejected', reason?: string) => {
    ...
    if (status === 'active') {
      const title = language === 'bn' ? 'বিজ্ঞাপন এপ্রুভ ও লাইভ হয়েছে! 🎉' : 'Ad Approved & Live! 🎉';
      ...
      const title = language === 'bn' ? 'বিজ্ঞাপন পর্যালোচনায় প্রত্যাখ্যাত হয়েছে (ইডিট প্রয়োজন) ⚠️' : 'Ad Needs Editing / Rejected ⚠️';
      const message = language === 'bn'
      ...
      const title = language === 'bn' ? 'বিজ্ঞাপন রিভিউতে রাখা হয়েছে ⏳' : 'Ad Sent to Review Queue ⏳';
      const message = language === 'bn'
        ? `আপনার "${targetTitle}" বিজ্ঞাপনটি রিভ্যুর জন্য জমা রাখা হয়েছে।`
        : `Your ad "${targetTitle}" is placed in review queue.`;
  ...
  const updateExistingAd = (id: string, adData: Partial<Product>) => {
        ...
        if (p.id === id) {
          ...
          const merged: Product = {
            ...
            seller: {
              ...p.seller,
              ...(adData.seller || {}),
              hidePhone: adData.seller?.hidePhone !== undefined ? Boolean(adData.seller.hidePhone) : p.seller?.hidePhone
            },
            status: 'pending',
    ...
    const editTitle = language === 'bn' ? 'বিজ্ঞাপন ইডিট করা হয়েছে (আন্ডার রিভিউ) ⏳' : 'Ad Re-submitted for Review ⏳';
