...
export const MarketProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [products, setProducts] = useState<Product[]>(() => {
    ...
    // Foundational 400 canonical listings
    INITIAL_PRODUCTS.forEach(p => {
      if (p && p.id) map.set(p.id, { ...p, status: 'active', isApproved: true, isActive: true });
    });
        ...
        if (Array.isArray(parsed) && parsed.length > 0) {
          parsed.forEach((p: Product) => {
            if (p && p.id && p.title && p.title.trim() !== '' && !['prod-1787413284395', 'prod-pending-2', 'prod-pending-3', 'prod-1787183789329'].includes(p.id)) {
              ...
              if (existing) {
                map.set(p.id, { ...existing, ...updatedP, status: updatedP.status || 'active' });
              } else {
