        ...
        <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 border border-gray-200 dark:border-slate-800 shadow-2xs space-y-5">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-3 pt-2">
            <div className="relative w-full sm:w-72">
              ...
              <input
                type="search"
                name="admin_activity_log_filter"
                id="admin-activity-log-filter"
                autoComplete="off"
                autoCorrect="off"
                autoCapitalize="none"
                spellCheck="false"
          ...
          <div className="divide-y divide-gray-100 dark:divide-slate-800">
            {(activityLogs || [])
              .filter(log => {
                ...
                const sTerm = String(activitySearch || '').toLowerCase().trim();
                const userNameStr = String(log.user || log.userName || '').toLowerCase();
                const targetStr = String(log.target || log.adTitle || '').toLowerCase();
                const actionStr = String(log.action || '').toLowerCase();
                const matchesSearch = !sTerm ||
                  userNameStr.includes(sTerm) ||
                  targetStr.includes(sTerm) ||
                  actionStr.includes(sTerm);
