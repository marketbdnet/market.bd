...
export const HomePage: React.FC = () => {
  ...
  // Display first 5 Featured/Urgent items consistently without unexpected rotation
  const displayedFeatured = featuredAds.slice(0, 5);
        ...
        <main className="flex-1 w-full min-w-0 space-y-6">
          ...
          <section className="bg-white dark:bg-slate-900 border-2 border-pink-500/70 dark:border-pink-500/60 rounded-2xl p-3.5 sm:p-5 shadow-xl space-y-4">
            ...
            {displayedFeatured.length > 0 ? (
              ...
              >
                {displayedFeatured.map((product, idx) => (
                  <ProductCard
                    key={product.id}
                    product={product}
                    layoutMode={adViewMode}
          ...
          <section className="bg-gradient-to-b from-sky-50/50 via-white to-white dark:from-slate-900/95 dark:via-slate-900 dark:to-slate-900 rounded-2xl border-2 border-indigo-500/70 dark:border-indigo-500/60 shadow-xl p-3.5 sm:p-5 space-y-4 relative overflow-hidden">
            ...
            {displayedRecent.length > 0 ? (
              ...
              >
                {displayedRecent.map((product, idx) => (
                  <ProductCard
                    key={product.id}
                    product={product}
                    layoutMode={adViewMode}
              ...
              <div className="flex items-center gap-2">
                {hasMoreRecent && (
                  ...
                  >
                    ...
                    <span>
                      {language === 'bn'
                        ? `আরও ${Math.min(10, filteredRecent.length - visibleRecentCount)} টি দেখুন`
                        : `Load ${Math.min(10, filteredRecent.length - visibleRecentCount)} More`}
                    </span>
                  </button>
                )}
