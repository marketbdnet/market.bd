...
export const SearchPage: React.FC = () => {
  ...
  const sortedProducts = [...scoredProducts].sort((a, b) => {
    ...
    if (filters.sortBy === 'popular') return (b.product.views || 0) - (a.product.views || 0);
    
    // Explicit 'latest' sorting: Newest posted ad strictly at the top
    if (filters.sortBy === 'latest') {
      const timeA = new Date(a.product.postedAt || 0).getTime();
      const timeB = new Date(b.product.postedAt || 0).getTime();
      return timeB - timeA;
    }

    // Default sorting: If search query relevance differs, sort by score, otherwise newest ad on top
    if (b.score !== a.score) {
