...
export async function submitManualPayment(
  ...
  try {
    ...
    if (!isFirestoreQuotaExceeded()) {
      try {
        await safeFirestoreSetDoc(doc(db, PAYMENTS_COLLECTION, paymentId), newPayment);
      } catch (err) {
...
export async function verifyPayment(
  ...
  try {
    ...
    if (!isFirestoreQuotaExceeded()) {
      try {
        await safeFirestoreSetDoc(doc(db, PAYMENTS_COLLECTION, paymentId), updatedPayment);

        // Synchronize Order status atomically if orderId exists
        if (payment.orderId) {
          try {
            const orderRef = doc(db, 'orders', payment.orderId);
            await safeFirestoreUpdateDoc(orderRef, {
