...
import { VERIFIED_LIVE_COMMUNITY_ADS } from './src/data/liveCommunityAds';
...
Respond in friendly, natural Bengali (à¦¬à¦¾à¦‚à¦²à¦¾) or English as requested. Provide concise price estimates in Bangladeshi Taka (à§³), smart deal tips, or generate high-converting ad descriptions for sellers. Keep formatting bulleted and easy to read.`;
  ...
  VERIFIED_LIVE_COMMUNITY_ADS.forEach(p => {
    if (p && p.id) canonicalMap.set(p.id, { ...p, status: 'active', isApproved: true, isActive: true });
  });
  ...
  try {
    ...
    if (fs.existsSync(fbConfigPath)) {
      ...
      // Sync baseline VERIFIED_LIVE_COMMUNITY_ADS to Firestore so all clients (Chrome, Firefox, Vercel, Web, Android) receive 418 authoritative ads in real-time
      VERIFIED_LIVE_COMMUNITY_ADS.forEach(async (ad) => {
        if (ad && ad.id) {
          try {
            await setDoc(doc(serverDb, 'products', ad.id), sanitizeForFirestore(ad), { merge: true });
          } catch (e) {}
        }
      });
  ...
  // Client automated version & health check endpoint for zero-refresh dynamic syncing
  app.get('/api/version', (_req, res) => {
    noCacheHeaders(res);
    res.json({
      success: true,
      version: 'v418_live',
      authoritativeCount: sharedSyncState.marketplace_products?.length || 418,
      timestamp: new Date().toISOString()
    });
  });
  ...
  // Universal User Single-Device Active Session Store (userId -> session object)
  const activeUserSessions: Record<string, { sessionToken: string; userIdentifier?: string; deviceName: string; ip: string; loginTime: string; updatedAt: string }> = {};

  app.get('/api/user/session/:userId', (req, res) => {
    const userId = req.params.userId;
    res.json({
      success: true,
      data: activeUserSessions[userId] || null,
      message: 'Active user session retrieved'
    });
  });

  app.post('/api/user/session', (req, res) => {
    try {
      const { userId, sessionToken, userIdentifier, deviceName, ip, loginTime, updatedAt } = req.body || {};
      ...
      activeUserSessions[userId] = {
        ...
        updatedAt: updatedAt || loginTime || new Date().toISOString()
      };

      res.json({
        success: true,
        message: 'User single-device session registered successfully',
        data: activeUserSessions[userId]
      });
    } catch (e: any) {
      res.status(500).json({ success: false, error: e?.message || 'Failed to update user session' });
    }
  });

  app.delete('/api/user/session/:userId', (req, res) => {
    const userId = req.params.userId;
  ...
  app.post('/api/sms-proxy/send', async (req, res) => {
    try {
      ...
      if (pLower.includes('bulksmsdhaka')) {
        // BulkSMSDhaka.net endpoint
        const endpoint = isOtp ? 'otpsend' : 'sendtext';
        const targetUrl = `https://bulksmsdhaka.net/api/${endpoint}?apikey=${encodeURIComponent(cleanKey)}&callerID=${encodeURIComponent(cleanSender)}&number=${encodeURIComponent(localPhone)}&message=${encodeURIComponent(cleanMsg)}`;

        const gatewayResponse = await fetch(targetUrl, { method: 'GET' });
        ...
        const targetUrl = `https://bulksmsbd.net/api/smsapi?api_key=${encodeURIComponent(cleanKey)}&type=text&number=${encodeURIComponent(localPhone)}&senderid=${encodeURIComponent(cleanSender)}&message=${encodeURIComponent(cleanMsg)}`;
        const gatewayResponse = await fetch(targetUrl);
        const data = await gatewayResponse.json();
        const isSuccess = data.response_code === 1001 || data.status === 'success';
        return res.json({
          success: isSuccess,
          message: data.success_message || data.error_message || 'SMS Processed',
          data
        });
      } else if (pLower.includes('greenweb')) {
        const targetUrl = `https://api.greenweb.com.bd/api.php?token=${encodeURIComponent(cleanKey)}&to=${encodeURIComponent('88' + localPhone)}&message=${encodeURIComponent(cleanMsg)}`;
        const gatewayResponse = await fetch(targetUrl);
        const text = await gatewayResponse.text();
        const isOk = text.includes('Ok') || text.includes('100');
        return res.json({
          success: isOk,
          message: isOk ? 'Greenweb SMS Sent Successfully' : text,
          data: text
        });
      } else {
        const base = pLower.startsWith('http') ? pLower : `https://${pLower}`;
        const targetUrl = `${base}/api/sendtext?apikey=${encodeURIComponent(cleanKey)}&callerID=${encodeURIComponent(cleanSender)}&number=${encodeURIComponent(localPhone)}&message=${encodeURIComponent(cleanMsg)}`;
        try {
          const gatewayResponse = await fetch(targetUrl);
          const text = await gatewayResponse.text();
          return res.json({ success: true, message: text || 'SMS Request Sent', data: text });
        } catch (postErr: any) {
          return res.json({ success: false, message: `Could not reach ${provider}: ${postErr.message}` });
        }
      }
    } catch (err: any) {
      return res.status(500).json({ success: false, message: err?.message || 'Server SMS Gateway Dispatch error' });
    }
  });

  app.get('/api/sms-proxy/balance', async (req, res) => {
    try {
      const { provider = 'bulksmsdhaka.net', apiKey } = req.query as { provider?: string; apiKey?: string };
      ...
      if (pLower.includes('bulksmsdhaka')) {
        ...
        if (parsed?.error) {
          return res.json({
            success: false,
            balance: '0',
            message: `BulkSMSDhaka: ${parsed.error}`
          });
        }
        return res.json({
          success: true,
      ...
      return res.status(500).json({ success: false, message: err?.message || 'Error checking balance' });
    }
  });

  // Promotional Email Proxy Dispatcher for Registered Customers
  app.post('/api/email-proxy/send', async (req, res) => {
    try {
      ...
      Object.keys(updates).forEach(key => {
        if (updates[key] !== undefined) {
          ...
          if (key === 'marketplace_products' || key === 'products') {
            ...
            const isAdminKey = req.headers['x-admin-key'] || req.headers['x-admin-token'];
            const hasActiveAdmin = sharedSyncState.adminActiveSession && (sharedSyncState.adminActiveSession.role === 'admin' || sharedSyncState.adminActiveSession.role === 'super_admin');
            if (isAdminKey || hasActiveAdmin || authHeader === 'Bearer marketbd-admin-token') {
              sharedSyncState.marketplace_products = updates[key];
              sharedSyncState.products = updates[key];
            } else {
              console.warn('[Server Security] Blocked unauthorized client attempt to overwrite marketplace_products via /api/sync/state');
            }
          }
        }
      });
  ...
  app.get('/api/products', (req, res) => {
    try {
      ...
      // Clean invalid stubs
      let productList = rawList.filter(p => p && p.id && p.title && p.title.trim() !== '' && !['prod-1787413284395', 'prod-pending-2', 'prod-pending-3', 'prod-1787183789329'].includes(p.id));
  ...
  // Bulk Product Sync endpoint - restricted to authenticated admin only
    ...
    try {
      ...
      if (!isAdminKey && !hasActiveAdmin && authHeader !== 'Bearer marketbd-admin-token') {
        return res.status(403).json({
          success: false,
          error: 'Forbidden: Automatic client bulk catalog overwrite is disabled for data consistency. Only authenticated admin sync is allowed.'
        });
      }
